...
New kits use a canonical `.cf-studio-kit.toml` manifest. The CLI normalizes that manifest into a `KitModel`, which drives install, update, validation, variable resolution, and generated host integrations. Older `manifest.toml` and directory-layout kits remain compatibility inputs.
...
| `.cf-studio/config/kits/{slug}/` or registered local kit paths | Tracked, ignored, or registered per kit |
...
| `.bootstrap/` | Self-hosted contributor-only context |
 ...
 |---|---|
 | `core.toml` | Top-level project settings, installed kits, kit resource bindings, and workspace registration |
