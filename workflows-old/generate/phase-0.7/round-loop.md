...
RULES:
  - ALWAYS REQUIRE `{cf-studio-path}/.core/workflows/shared/inline-fallback-probe.md` loaded before any cf-* sub-agent dispatch
  - ALWAYS REQUIRE INLINE_FALLBACK set before first round (probed by phase-0-dependencies.md)
  - ALWAYS IF INLINE_FALLBACK unset at any round-level dispatch site (e.g. after context-loss):
  ...
  - NEVER re-probe per-round when INLINE_FALLBACK is already set
```
...
STATE:
  - SET panel_mode: single-agent | fan-out   default: single-agent   scope: per-round
NOTES:
...
RULES:
  - ALWAYS WHEN panel_mode=="single-agent" AND len(state.panel)>1:
      NEVER dispatch one cf-brainstorm-expert per panel member (SILENT_MODE_DOWNGRADE)
  - ALWAYS WHEN panel_mode=="fan-out":
      NEVER dispatch cf-brainstorm-panel
ON_ERROR:
...
STATE:
  - SET pending_round_kind: topic | challenge   default: topic
  - SET INLINE_FALLBACK_THIS_ROUND: bool        default: false   scope: per-round

DO:
  - RUN WHILE state.topic_current is not None:
...
RULES:
  - ALWAYS emit pre-dispatch checkpoint after availability check resolves agent AND before dispatch tool call
  - NEVER emit checkpoint when availability check routes to its 3-option menu
  - ALWAYS Omitting checkpoint is MISSING_DISPATCH_CHECKPOINT failure: STOP, re-emit checkpoint, then continue
  - ALWAYS INLINE_FALLBACK_THIS_ROUND is per-round scope; see sub-agent-dispatch.md § Registered native sub-agent set & INLINE_FALLBACK_THIS_ROUND
```
...
DO:
  - RUN VERIFY resolved agent is in host registered native sub-agent set

  - REQUIRE agent unavailable AND INLINE_FALLBACK == false:
...
DO:
  ...
  - REQUIRE panel_mode == "fan-out":
    - DISPATCH [cf-brainstorm-expert for each e in state.panel]
      WITH: parallel=true, persona=e, topic=state.topic_current, state,
            resource_context=state.resource_context, mode="topic"
    - SET contributions = all_results

  - RUN otherwise # single-agent
...
DO:
  ...
  - RUN otherwise # single-agent; state.rounds guaranteed non-empty by precondition
    - CONTINUE Phase07MidSessionMutationGuard
    IF status != "skipped":
      - DISPATCH cf-brainstorm-panel
        WITH: panel=state.panel, topic=state.topic_current, state,
...
DO:
  - REQUIRE state.rounds[-1].protocol AND state.rounds[-1].protocol != protocol:
    - SET status = "skipped"
    - SET health = { degraded: true, reason: "protocol changed mid-session" }
    - SET contributions = []
  - RUN ELIF state.rounds[-1].panel != state.panel:
    - SET status = "skipped"
    - SET health = { degraded: true, reason: "panel mutated mid-session" }
    - SET contributions = []

RULES:
  - ALWAYS be called by single-agent branches of Phase07TopicDispatch and Phase07ChallengeDispatch when state.rounds is non-empty
  - NEVER dispatch any agent when status == "skipped"
  - ALWAYS Callers ALWAYS check status != "skipped" before proceeding to dispatch
```
...
STATE:
  - SET attempts_used: int   default: 1   scope: per-round

DO:
  - RUN validate_contributions(contributions, kind=pending_round_kind,
                             challenged_decisions=challenged_decisions)

  - RUN # Structural invariants (short-circuit; no repair):
  - RUN #  1. contributions not a list  2. contribution has no expert_id
  - RUN #  9. contribution has questions but no relevant field  12. (single-agent) invalid envelope kind
  - REQUIRE structural_errors:
    - SET status = "skipped"
    - SET health = { degraded: true, reason: "Structural invariant violation {structural_errors[0]}", attempts_used: 1 }
    - SET contributions = []
    - RETURN

  - RUN # Content invariants (accumulate all; repair once):
  - RUN #  3. relevant contribution has no questions array  4. question has no decision_key
  - RUN #  5. empty decision_key  6. duplicate decision_key (topic-round)
  - RUN #  7. challenge decision_key not in challenged_decisions  8. question has no text
  - RUN #  10. relevant=false but no reason  11. topic-round relevant contribution has no next_topic_proposal
  - REQUIRE content_errors AND attempts_used == 1:
    - SET repair_feedback = { mode, panel_mode, protocol, violations, prior_contributions }
    - DISPATCH previous validation target WITH repair_feedback signal
    - SET attempts_used = 2
    RE-VALIDATE
  ...
  - REQUIRE attempts_used >= 2 AND (structural_errors OR content_errors):
    - SET status = "skipped"
    - SET health = { degraded: true, reason: "{N} invariant violations; retry limit reached", attempts_used: attempts_used }
    - SET contributions = []
  - RUN ELIF NOT (structural_errors OR content_errors):
    - SET status = "ok"
...
DO:
  ...
  - RUN # challenge-rounds reuse the most recent topic-round's proposals
...
RULES:
  - ALWAYS validate every participating topic-round question has non-empty decision_key
  - ALWAYS reject duplicate topic-round decision_key values as malformed expert output
  - ALWAYS parse-side guard: challenge choices are valid only when rounds[-1].answer_keys is non-empty
  - NEVER render critique-only challenge outputs as skipped (keep in participating)
  - NEVER render critique-only challenge outputs as skipped
  - NEVER ask the user to answer the whole question_queue in one reply
```
...
RULES:
  - ALWAYS show exactly one pending question per turn
  - NEVER dump full question_queue to user
  - NEVER dump the full question_queue to the user
  - ALWAYS include concise context per question: expert/persona, why it matters, proposed default, relevant critique
  - ALWAYS offer numbered answer options for each question
```
...
RULES:
  - ALWAYS option 4 ALWAYS be hidden in topic-rounds; reply 4 in topic-round re-renders menu
  - ALWAYS "accept"/"yes"/"default" -> option 1; "skip" -> option 3; "keep" -> option 4 (challenge only); "W"/"wrap"/"save"/"stop"/"enough"/"done" -> option 5
  - ALWAYS After each recorded reaction update rounds[-1].answers and rounds[-1].answer_keys immediately
  - ALWAYS answer_keys includes only accepted-default/custom topic answers and accepted-default/custom challenge overwrites; skip/keep excluded
  - ALWAYS skip NEVER update state.decisions
  - ALWAYS skip NEVER update state.decisions
  - ALWAYS skip ALWAYS preserve question as open_unanswered for downstream PRD/DESIGN/ADR/FEATURE
  - ALWAYS always show wrap/save as user-facing option
...
MENU SkippedRoundMenu:
  ...
  OPTIONS:
    ...
    2 R (without confirm or malformed) ->
...
RULES:
  - NEVER show before every pending question is resolved
  - NEVER show this menu before every pending question is resolved
  - ALWAYS always show W / wrap as user-facing option
```
...
RULES:
  - NEVER mutate state on malformed reply (multiple tokens, unknown letter, C when hidden)
  - ALWAYS re-render post-round menu prefixed with one-line clarifier on malformed reply
  - ALWAYS assert rounds[-1].answer_keys is a list; if not, treat as malformed-state error and re-ask

  - ALWAYS SWITCH user choice:
    W | stop_token ->
      SET state.topic_current = None
...
MENU RoundCapMenu:
  ...
  OPTIONS:
    1 extend: M (M positive integer > current BRAINSTORM_MAX_ROUNDS) ->
...
RULES:
  - ALWAYS Every user-facing brainstorm menu after brainstorm acceptance ALWAYS expose a W or wrap option
  - ALWAYS Wrap ALWAYS route to {cf-studio-path}/.core/workflows/generate/phase-0.7/wrap-handoff.md
  - ALWAYS Wrap NEVER imply discard; wrap-handoff owns save/generate/analyze/continue routing
```
...
DO:
  - RUN INITIALIZE contributions = []
  - RUN FOR each block in envelope.blocks WHERE block.kind == "independent":
  ...
  - RUN FOR each block in envelope.blocks WHERE block.kind == "critique":
    FOR each row in block.rows:
...
RULES:
  - ALWAYS Fan-out (panel_mode="fan-out"):
    - ALWAYS open, load, and follow {cf-studio-path}/.core/skills/studio/agents/cf-brainstorm-expert.md per expert dispatch
    - ALWAYS Final prompt ALWAYS preserve canonical input payload, output contract, parse-time invariants, completion gate, final emit instruction
    - ALWAYS Fail-closed per sub-agent-dispatch.md § SubAgentContractReadGate if contract missing/unreadable/ambiguous
    - ALWAYS Per-expert per-round fields: persona, topic, mode, challenged_decisions (required when mode="challenge"; omit/null when mode="topic"), repair_feedback (optional)
    - ALWAYS state sub-fields ALWAYS: kind, rules_loaded, kit_rules_path, template_path, panel, decisions, topic_history, resource_context
    - ALWAYS state sub-fields OPTIONAL when available: example_path, rounds, open_questions, session_id, next_topic_proposals

  - ALWAYS Single-agent (panel_mode="single-agent"):
    - ALWAYS open, load, and follow {cf-studio-path}/.core/skills/studio/agents/cf-brainstorm-panel.md
    - ALWAYS Final prompt ALWAYS preserve canonical contract sections for input, envelope shape, parse-time invariants, completion gate
    - NEVER replace canonical schema with a short bullet summary when it can be carried forward directly
    - ALWAYS Fields: panel, topic, resource_context, mode, protocol, challenged_decisions (required on mode="challenge"), repair_feedback (optional), state (same as fan-out)
```
