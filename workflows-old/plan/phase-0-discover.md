...
NOTES:
  Resolved values must be carried forward to Phase 3.1, where they ALWAYS be
...
RULES:
  - ALWAYS write {cfs_cmd}, {cf-studio-path}, and {project_root} into plan.toml [meta]
    table in Phase 3.1 so the runtime can read them on resume
  - ALWAYS re-run {cfs_cmd} --json info on context loss or new-chat resume to verify
    or refresh resolved values before any path-dependent step
  - ALWAYS parse plan.toml [meta] table first on resume, then re-run {cfs_cmd} --json info
...
DO:
  - RUN READ {cf-studio-path}/.core/skills/studio/studio.clispec
  - RUN FOR each COMMAND block:
    ADD to tool_map: {command_name} — {DESCRIPTION line} [outputs: {OUTPUT format}]
  - RUN FOR each script directory in {variables} (e.g. {scripts} when present):
