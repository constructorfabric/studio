"""
Usage Telemetry for Constructor Studio CLI Proxy
...
@cpt-dod:cpt-studio-dod-core-infra-telemetry:p1
"""
...
LOG_DIR = Path.home() / ".cf-studio" / "logs"
...
def track_invocation(args: List[str]) -> None:
    ...
    2. Appends a JSONL record to ~/.cf-studio/logs/YYYY-MM-DD.log
    3. Rotates old log files if a new day's file was just created
    4. POSTs OTLP Logs JSON to CFS_TELEMETRY_URL (if set)
    5. Logs HTTP errors to the same log file

    Disabled entirely when CFS_TELEMETRY=0.
    Skipped for --version and --help (no useful data, fast exit risk).
    """
    if os.environ.get("CFS_TELEMETRY") == "0":
        return
...
def _telemetry_worker(command: str) -> None:
    ...
    try:
        ...
        record = {
            ...
            "command": command,
            "constructor_studio_version": __version__,
        }
        ...
        telemetry_url = os.environ.get("CFS_TELEMETRY_URL")
...
def _build_otlp_logs(
    ...
    return {
        "resourceLogs": [{
            "resource": {
                "attributes": [
                    _str_attr("service.name", "constructor-studio"),
                    _str_attr("service.version", version),
                ],
            },
            "scopeLogs": [{
                "scope": {"name": "studio.telemetry"},
                "logRecords": [{
                    ...
                    "attributes": [
                        ...
                        _str_attr("studio.command", record.get("command", "")),
                    ],
                }],
            }],
        }],
