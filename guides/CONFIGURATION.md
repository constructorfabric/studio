...
- **installed kits** are tracked or ignored per kit under `config/kits/{slug}/`
...
## 8. Kit Management
 ...
 Kits are direct file packages that provide templates, rules, checklists, constraints, workflows, scripts, and skill surfaces under `config/kits/{slug}/`. Each kit has its own git tracking policy. Tracked kits are editable repository content and get file-level diffs on update. Ignored kits are generated local content and may be overwritten by Studio repair/update flows.
