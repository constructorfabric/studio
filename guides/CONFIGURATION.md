...
`cfs generate-agents` regenerates host integration files. `cfs update` repairs/updates the Studio runtime and generated integrations, but it does not update kit files unless you explicitly opt in with `--with-kits yes`. Use `cfs kit update` when you want upstream kit changes.
