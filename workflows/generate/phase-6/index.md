...
WHEN:
  - REQUIRE LOAD this phase after Phase 5 exit when at least one is true:
    - REQUIRE Validation Results body exists or must be surfaced
    - REQUIRE waiver or waiver-exception state must be evaluated or rendered
    - REQUIRE files were written or changed
    - REQUIRE remaining_findings is non-empty
    - REQUIRE validation state is unresolved, including any outstanding validation or
      waiver decision
  - REQUIRE SKIP this phase only when all are true:
    - REQUIRE output is chat-only
    - REQUIRE no files changed
    - REQUIRE remaining_findings is empty
    - REQUIRE no outstanding validation or waiver decision remains

  - REQUIRE NOTE outstanding validation or waiver decision:
...
RULES:
  - ALWAYS Late-phase routing relies on controller-supplied prompt_context_view slices
  - NEVER reopen prompt assets from disk
  - ALWAYS If validation prerequisites, waiver applicability, or terminal clean/dirty
...
DO:
  ...
  - RUN NOTE: requirement (2) is waived by the first matching exception:
  ...
  - RUN WAIVER PRIORITY (first match wins when multiple apply):
    - det_gate_final_result == "FAIL"
    - external entry from analyze.md with MAX_ITER=0
    - PARTIAL_CHECKPOINT

  - RUN PARTIAL_CHECKPOINT impact:
    Phase 6 ALWAYS keep remaining_findings non-empty
    ALWAYS surface checkpoint/resume data through remediation handoff
    - NEVER emit clean post-write handoff until checkpoint is resumed and completed

  - REQUIRE required output is missing or contains placeholder/template content
     outside these exceptions:
    ABORT with clear prerequisite error stating which Phase 5 output is missing
    - NEVER emitting handoff menus

RULES:
  - ALWAYS Any unresolved question about whether requirement (2) is satisfied or
  ...
  - ALWAYS Consume Phase 5 outputs and waiver state from controller-supplied
    prompt_context_view slices; NEVER reopen prompt assets from disk
...
DO:
  ...
  - REQUIRE rules.md available AND has ## Next Steps:
    - EMIT exactly:
- RUN ---
- RUN What would you like to do next?
- RUN Review written files — Invoke skill `cf-analyze` on the generated or modified files. Suggested when files changed and confidence matters.
- RUN Continue generating — Invoke skill `cf-generate` using the current results as context.
- RUN Plan follow-up work — Invoke skill `cf-plan` when remaining work is broad or multi-step.
- RUN Done — end the generate session with no further workflow handoff.
- RUN Other — say what you want to change or do next.
- RUN Reply with the option number or a short custom instruction.
- RUN ---

  - RUN otherwise (rules.md unavailable or no ## Next Steps):
    - EMIT exactly:
- RUN ---
- RUN What would you like to do next?
- RUN Invoke skill `cf-analyze` on the written files — Suggested when files were created; validates the output and surfaces any remaining issues.
- RUN Done — end the generate session with no further workflow handoff.
- RUN Other — describe the next action.
- RUN Reply with the option number or a short custom instruction.
- RUN ---

RULES:
  - ALWAYS Next-step menu is informational only when phase-4-write.md wrote or updated files
  - ALWAYS emit next-step menu before the terminal handoff section
```
...
DO:
  - REQUIRE remaining_findings is non-empty:
    ...
    - NEVER emitting W replies in this response
    - NEVER emitting post-write-handoff.md while remediation pending
    NOTE: Post-write review choices deferred until remediation is processed

  - REQUIRE remaining_findings is empty AND files were written:
    - LOAD post-write-handoff.md as terminal section

  - REQUIRE output was chat-only AND no files changed AND remaining_findings is empty
     AND no outstanding validation or waiver decision remains:
    SKIP both handoff menus

  - REQUIRE chat-only output with non-empty remaining_findings:
    - EMIT remediation-handoff.md as terminal actionable menu
    NOTE: no-files condition does NOT suppress remediation

  - RUN AFTER terminal handoff sub-file emits its final section:
    Generate workflow is COMPLETE — no further phase loads

RULES:
  - NEVER ask whether handoff menus should be generated
  - NEVER defer required remediation handoff to a later user turn
  - ALWAYS emit remediation handoff when remaining_findings is non-empty
  ...
  - ALWAYS Phase 6 skip is allowed only on the chat-only clean path with no files
    changed, no findings, and no outstanding validation/waiver decision
  - ALWAYS If files were written and remaining_findings is empty:
    ALWAYS emit Post-Write Review Handoff menu as FINAL section;
    omitting it makes generate output incomplete
  - ALWAYS If files were written and remaining_findings is non-empty:
    ALWAYS emit Remediation Handoff menu as terminal section;
    asking for actionable W replies makes generate output incomplete
  - ALWAYS A summary alone is not completion
  - ALWAYS Validation Results body alone is not completion
  - ALWAYS Next-step menu alone is not completion

INVARIANTS:
  - ALWAYS Fix Prompt / Plan Prompt / Direct Review Prompt / Plan Review Prompt
    ALWAYS be emitted ONLY on the NEXT turn after user picks matching handoff option;
    NEVER emit in same turn as handoff menu
