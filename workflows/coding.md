...
PURPOSE: After edits, run a semantic review at the user-chosen granularity and iterate fixes until the code is clean.
STATE:
  SET REVIEW_GRANULARITY: single-pass | per-methodology | per-layer (default unset, scope workflow_run)
WHEN:
  REQUIRE edits have been applied to the code
DO:
  ...
  CONTINUE CodingValidate WHEN review fixes were applied this iteration (re-run the deterministic gate before re-reviewing)
...
RULES:
  ...
  NEVER declare code done until BOTH the deterministic gate (tests/lint/typecheck/build) passes AND the semantic review has no remaining findings; ALWAYS re-run CodingValidate after any fix before re-reviewing
