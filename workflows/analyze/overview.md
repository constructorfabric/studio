...
Skill invocations: Invoke skill `cf-analyze`, Invoke skill `cf-analyze` with semantic mode, Invoke skill `cf-analyze` with `--artifact <path>`, Invoke skill `cf-analyze` with semantic mode and `--artifact <path>`.
...
If analysis finds actionable issues, the workflow MUST end with the `Remediation Handoff` menu offering three paths: (1) continue in this session with Invoke skill `cf-generate` in fix mode (suggested when scope is bounded), (2) generate a self-contained `Fix Prompt` for a fresh chat via Invoke skill `cf-generate`, (3) generate a self-contained `Plan Prompt` for a fresh chat via Invoke skill `cf-plan`. The two prompt blocks (Fix / Plan) are emitted on demand only when the user picks option 2 or 3 in the next turn; both, when emitted, MUST be self-contained — all findings, paths, and context embedded inline.
