...
skills available to the current session (via the host agentic tool), matches the
...
```pdsl
UNIT AnalyzeRoute
PURPOSE: Capture the analyze intent, discover session cf-* skills via the host, and route to the chosen skill.
STATE:
...
DO:
  ...
  RUN ask the host agentic tool for the list of cf-* skills available to this session; SET AVAILABLE_SKILLS = the returned skills (name + short description), excluding this router (cf-analyze); SET AVAILABLE_SKILLS = empty when the host returns nothing, errors, or is unavailable
