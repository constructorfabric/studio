...
Common steps shared by `{cf-studio-path}/.core/workflows/generate.md` and `{cf-studio-path}/.core/workflows/analyze.md`. Both workflows MUST execute this protocol before their specific logic.
...
| ❌ Not running `{cfs_cmd} info` | Discard invalid output |
| ❌ Not following invoked workflow rules (`{cf-studio-path}/.core/workflows/generate.md` / `{cf-studio-path}/.core/workflows/analyze.md`) | Restart: re-run protocol + show compliance report |
...
**Agent MUST NOT**:
- Continue Constructor Studio work without re-loading specs after compaction
- Assume specs are "still loaded" from before compaction
- Skip protocol because "it was already done"

## Deterministic Operations

**ALWAYS** run `{cfs_cmd} --json toc <file>` for ANY Table of Contents generation or update in Markdown files. **NEVER** write TOC manually — agent-generated anchors are unreliable. Manually written TOC = **INVALID output**.

## Mode Detection

### Constructor Studio Mode

| Topic | Required behavior |
|---|---|
| Default | Treat request as workflow execution ONLY when Constructor Studio is enabled |
| Enable | User invoking Constructor Studio workflow (`/cfs`, `/prd`, `/design`, etc.) = Constructor Studio enabled |
| Disable | User requesting `/cf off` = Constructor Studio disabled for conversation |
| Disabled behavior | When disabled, behave as normal coding assistant |

Announce Constructor Studio mode (non-blocking):

```text
Constructor Studio mode: ENABLED. To disable: /cf off
```

### Rules Mode

After Constructor Studio discovery, determine **Rules Mode**:

| Mode | Detect when | Required behavior | Next step / message |
|---|---|---|---|
| **STRICT** | `artifacts.toml` found AND contains `rules` section AND target artifact/code matches registered system | Full protocol enforcement; mandatory semantic validation; evidence requirements enforced; anti-pattern detection active; Agent compliance protocol applies (see `{cf-studio-path}/.core/requirements/agent-compliance.md`) | Announce `Rules Mode: STRICT (sdlc rules loaded)` and `→ Full validation protocol enforced` |
| **BOOTSTRAP** | Constructor Studio found AND `artifacts.toml` has empty `systems[].artifacts` array | See BOOTSTRAP requirements below | Welcome + propose first artifact |
| **RELAXED** | No Constructor Studio found OR no `kits` in `artifacts.toml` | ALWAYS propose initialization; ALWAYS proceed as normal coding assistant WHEN user declines initialization | `Constructor Studio not configured` then `→ cf init to initialize for this project` |
...
### Discover Constructor Studio

```bash
{cfs_cmd} --json info --root {project_root} --cf-root {cf-studio-path}
```

Parse JSON output: `status`, `relative_path`, `project_root`, `specs`, `rules`, `variables`.
...
- If FOUND: load `{cf-studio-path}/.gen/AGENTS.md` for navigation rules
- If NOT_FOUND: suggest running `cf init` to bootstrap

### Registry, Intent, and Kit Loading

| Phase | Required Steps | Stored Context |
| --- | --- | --- |
| **Understand Registry** | MUST read `{cf-studio-path}/config/artifacts.toml`; identify rules, systems, artifacts, and codebase; MUST inspect the registered kit package path(s) from `artifacts.toml.kits[*].path`, including `artifacts/` and `codebase/` | rules+paths, systems, artifact kinds, traceability settings |
...
| **Determine Artifact Type** | Resolve from explicit parameter or registry lookup: `cf generate PRD` → PRD; `cf analyze {path}` → `artifacts.toml.systems[].artifacts[path].kind`; path in `codebase[]` → CODE | `ARTIFACT_TYPE` |
...
| **Load Config Specs** | After rules loaded and target type determined, read `{cf-studio-path}/.gen/AGENTS.md`; use the matching rules below to open applicable specs; if config uses legacy `WHEN executing workflows: ...`, map workflow names to artifact kinds internally | `CONFIG_SPECS` |

Match config specs in this order:
1. Match `{rule}` to the loaded rules ID (for example `studio-sdlc`)
...
|---|---|---|
| Constructor Studio Not Found | `⚠️ Constructor Studio not configured` → `Run cf init to initialize` | STOP |
| `artifacts.toml` Parse Error | `⚠️ Cannot parse artifacts.toml: {parse error}` → `Fix TOML syntax errors in {cf-studio-path}/config/artifacts.toml` → `Validate with: python3 -c 'import tomllib; tomllib.load(open("artifacts.toml", "rb"))'` | STOP |
| `rules.md` Not Found | `⚠️ Rules file not found: {KITS_PATH}` → verify kit exists at `{KIT_BASE}` → check `artifacts.toml` kits path → run `cf init --force` to regenerate | STOP |
...
| System Not Registered | `⚠️ System not found: {system_name}` → show registered systems → options: `1. Register system via cf init  2. Use existing system  3. Continue in RELAXED mode` | Prompt user to choose |
...
|---|---|
| **Detection (D)** | D.1 Constructor Studio mode detected (agent states Constructor Studio enabled); D.2 Rules mode determined (STRICT/RELAXED + reason) |
| **Discovery (DI)** | DI.1 Constructor Studio discovery executed (`{cfs_cmd} info`); DI.2 `artifacts.toml` read/understood (agent lists systems/rules); DI.3 Rules directories explored (agent lists artifact kinds) |
