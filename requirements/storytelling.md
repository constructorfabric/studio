...
| 37 | Pausing the storytelling thread on a generate-routing dispatch (waiting synchronously for generate-skill completion before emitting the next portion) | Dispatch is fire-and-forget. The orchestrator emits a same-turn ack (`🚀 routed Q-N to generate skill — session-id …`) before the next portion's nav block, then continues navigation. Results arrive in their own turns with `[generate-route Q-N]:` prefix and MUST NOT merge into a portion body. Synchronous blocking defeats interactive cadence. |
