 ...
 **ALWAYS open and follow** `{cf-constructor-path}/.core/requirements/reverse-engineering.md` for scan methodology (`L1-L3`, `L8`).
 
 **ALWAYS open and follow** `{cf-constructor-path}/.core/requirements/prompt-engineering.md` for rule-quality validation.
 ...
 **Prerequisites**: confirm the agent has read this methodology, has source access, will follow phases `1 -> 6` in order, will checkpoint after each phase, and will **NOT** write files without user confirmation.
 ...
 Auto-config scans a brownfield project and generates **per-topic** rule files in `{cf-constructor-path}/config/rules/` plus contextual `WHEN` rules in `{cf-constructor-path}/config/AGENTS.md`. Files are split by **topic** (`conventions`, `architecture`, `patterns`, etc.), not by system path, so the agent loads only the guidance relevant to the current activity.
 
 **Core principle**: extract conventions from code, do not impose them.
 
 | Output | Location | Purpose |
 |---|---|---|
 | Per-topic rule files | `{cf-constructor-path}/config/rules/{topic}.md` | Focused rules per semantic topic |
 | Doc navigation rules | `{cf-constructor-path}/config/AGENTS.md` | `WHEN` rules for existing project docs with heading anchors |
 | Rule-file navigation rules | `{cf-constructor-path}/config/AGENTS.md` | `WHEN` rules that load generated topic files contextually |
 | Registry entries | `{cf-constructor-path}/config/artifacts.toml` | Detected systems with source paths |
 ...
 **Trigger conditions** (`ANY`):
 ...
 - Manual invocation via `cf-constructor auto-config` or equivalent user request
 - Rescan via `{cfc_cmd} init --rescan` or equivalent reconfigure request
 
 **Pre-checks**:
 - [ ] Cypilot is initialized (`cypilot.py info` returns `FOUND`)
 - [ ] Source-code repository is accessible
 - [ ] `{cf-constructor-path}/config/` exists and is writable
 - [ ] `{cf-constructor-path}/config/rules/` is empty, or `--force` is explicitly in use

 ## Phase 1: Project Scan
 
 **Goal**: extract raw project data using reverse-engineering methodology.
 
 **Use**: `{cf-constructor-path}/.core/requirements/reverse-engineering.md` Layers `1`, `2`, `3`, and `8`.
...
**Scan + capture**:
- [ ] Search `docs/`, `documentation/`, `guides/`, `wiki/`, `.github/`, standalone guides, ADR dirs, API docs, `README.md` links, `architecture/`, and `{cf-constructor-path}/config/`
...
**TOC flow**: offer `{cfc_cmd} toc {doc_path}`; show missing-TOC docs as `# | File | Headings | Topic`; on approval, run `{cfc_cmd} toc` per selected doc and verify success.
...
**Quality gate**: apply `{cf-constructor-path}/.core/requirements/prompt-engineering.md` Layer `2` and Layer `5` to every generated rule.
...
**Generation protocol**: 1) filter by topic 2) merge/skip if `<3` rules 3) synthesize 4) validate against `AP-VAGUE`, `AP-CONTEXT-BLOAT`, `AP-HALLUCINATION-PRONE` 5) present batch to user 6) write after confirmation 7) run `python3 {cf-constructor-path}/.core/skills/cypilot/scripts/cypilot.py toc {rule_file_path}`.
...
```markdown
ALWAYS open and follow `{cf-constructor-path}/config/rules/conventions.md` WHEN writing or reviewing code
ALWAYS open and follow `{cf-constructor-path}/config/rules/architecture.md` WHEN modifying architecture, adding components, or refactoring module boundaries
ALWAYS open and follow `{cf-constructor-path}/config/rules/patterns.md` WHEN implementing features or writing business logic
ALWAYS open and follow `{cf-constructor-path}/config/rules/testing.md` WHEN writing or running tests
ALWAYS open and follow `{cf-constructor-path}/config/rules/api-contracts.md` WHEN writing API endpoints or CLI commands
ALWAYS open and follow `{cf-constructor-path}/config/rules/anti-patterns.md` WHEN reviewing code or refactoring
```
...
**Goal**: register detected systems in `{cf-constructor-path}/config/artifacts.toml`.
...
```json
{"status":"PASS","systems_detected":2,"topics_generated":["conventions","architecture","patterns","testing"],"agents_rules_added":4,"registry_entries_added":2,"files_written":["{cf-constructor-path}/config/rules/conventions.md","{cf-constructor-path}/config/rules/architecture.md","{cf-constructor-path}/config/rules/patterns.md","{cf-constructor-path}/config/rules/testing.md"],"docs_found":3,"docs_toc_generated":2,"doc_navigation_rules_added":5}
```
...
- [ ] Validate with `python3 {cf-constructor-path}/.core/skills/cypilot/scripts/cypilot.py toc {rule_file_path}`
...
|---|---|---|
| No source code found | `No source code detected in project` → use `cf-constructor generate` for greenfield work. | **STOP** |
| Existing rules found | `Existing rules found in {cf-constructor-path}/config/rules/` → list files → use `--force` or merge manually. | **STOP** unless `--force` |
...
| Large codebase | `Large codebase detected ({file_count} files)` → scan top-level structure only → offer `cf-constructor auto-config --system {slug}`. | Limit scan depth |

## References

- Reverse Engineering: `{cf-constructor-path}/.core/requirements/reverse-engineering.md`
- Prompt Engineering: `{cf-constructor-path}/.core/requirements/prompt-engineering.md`
- Execution Protocol: `{cf-constructor-path}/.core/requirements/execution-protocol.md`
- Generate Workflow: `{cf-constructor-path}/.core/workflows/generate.md` (triggers auto-config in Brownfield prerequisite)
