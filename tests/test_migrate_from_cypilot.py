...
def test_generate_agents_preserves_cypilot_agent_configs_by_default(tmp_path):
    """Plain `generate-agents` must not remove coexisting Cypilot agent configs."""
    import studio.commands.agents as agents_mod

    (tmp_path / ".git").mkdir()
    studio_dir = tmp_path / ".cf-studio"
    (studio_dir / "skills" / "studio").mkdir(parents=True)
    (studio_dir / "skills" / "studio" / "SKILL.md").write_text(
        "---\nname: cf\ndescription: Constructor Studio\n---\n",
        encoding="utf-8",
    )
    legacy_agent = tmp_path / ".github" / "agents" / "cypilot-analyze.agent.md"
    legacy_agent.parent.mkdir(parents=True)
    legacy_agent.write_text(
        "<!-- Generated by cypilot agents -- do not edit -->\n"
        "ALWAYS open and follow `{cypilot_path}/.core/skills/cypilot/agents/cypilot-analyze.md`\n",
        encoding="utf-8",
    )

    result = agents_mod._process_single_agent(
        "copilot",
        tmp_path,
        studio_dir,
        agents_mod._default_agents_config(),
        None,
        dry_run=False,
    )
    ...
    assert ".github/agents/cypilot-analyze.agent.md" not in result["skills"]["deleted"]


def test_generate_agents_remove_cypilot_deletes_cypilot_agent_configs(tmp_path):
    """`--remove-cypilot=yes` keeps the explicit migration-style cleanup path."""
    import studio.commands.agents as agents_mod
    ...
    assert ".github/agents/cypilot-analyze.agent.md" in result["skills"]["deleted"]
