...
class TestKitUpdateCheckCoverage(unittest.TestCase):
    ...
    def test_public_component_name_conflicts_against_existing_registered_kit(self):
        ...
        with TemporaryDirectory() as td:
            ...
            ), patch("studio.utils.kit_model.load_installed_kit_model", return_value=existing):
    ...
    def test_normalize_multi_kit_defaults_to_all_kits(self):
        from studio.commands.kit import cmd_kit_normalize
        ...
        with TemporaryDirectory() as td:
            ...
            out = json.loads(buf.getvalue())
            self.assertEqual(out["status"], "PASS")
            self.assertEqual(out["kits"], ["alpha", "beta"])
            self.assertEqual(out["kits_normalized"], 2)
            data = tomllib.loads(out["manifest"])
            self.assertEqual([kit["slug"] for kit in data["kits"]], ["alpha", "beta"])

    def test_normalize_multi_kit_can_select_specific_kit(self):
        from studio.commands.kit import cmd_kit_normalize
        ...
        with TemporaryDirectory() as td:
            ...
            buf = io.StringIO()
            with redirect_stdout(buf):
                rc = cmd_kit_normalize([str(kit_src), "--dry-run", "--kit", "beta"])
            ...
            data = tomllib.loads(out["manifest"])
            self.assertEqual([kit["slug"] for kit in data["kits"]], ["beta"])

    def test_normalize_multi_kit_rejects_unknown_selection(self):
        from studio.commands.kit import cmd_kit_normalize
        ...
        with TemporaryDirectory() as td:
            ...
            buf = io.StringIO()
            with redirect_stdout(buf):
                rc = cmd_kit_normalize([str(kit_src), "--dry-run", "--kit", "gamma"])

            self.assertEqual(rc, 2)
            out = json.loads(buf.getvalue())
            self.assertEqual(out["status"], "FAIL")
            self.assertIn("Unknown kit selection", out["message"])
            self.assertIn("alpha", out["message"])
            self.assertIn("beta", out["message"])

    def test_normalize_multi_kit_subset_write_requires_explicit_output(self):
        from studio.commands.kit import cmd_kit_normalize
        ...
        with TemporaryDirectory() as td:
            ...
            buf = io.StringIO()
            with redirect_stdout(buf):
                rc = cmd_kit_normalize([str(kit_src), "--kit", "alpha"])
    ...
    def test_kit_model_can_disable_public_name_prefix_per_resource(self):
        from studio.utils.kit_model import load_kit_model, kit_models_to_toml_data
        ...
        with TemporaryDirectory() as td:
            ...
            self.assertFalse(kit_models_to_toml_data([model])["kits"][0]["resources"][0]["prefix_generated_name"])
    ...
    def test_kit_model_render_preserves_tool_declarations(self):
        ...
        with TemporaryDirectory() as td:
            ...
            data = kit_models_to_toml_data([load_kit_model(kit_src)])
