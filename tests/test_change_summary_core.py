...
class TestPrivacy:
    ...
    #: The one account name exempted, and only because it was **measured**. `make ci` runs
    #: the workflows through `act`, whose default runner image sets the variable this check
    #: reads --
    #:
    ...
    #: failed and `make ci` could not pass locally for anyone. (A bare `ubuntu:22.04` leaves
    #: `USER` empty, which is why it reproduces under `act` and not under every container.)
    #:
    #: **Narrowed from six names to one.** The first version also exempted `user`, `admin`,
    #: `runner`, `build` and `nobody`, none of which was ever observed anywhere. Review
    #: pointed out that a plain membership test cannot tell a container default from a real
    #: person's account, and that is right: a developer whose username happens to be `build`
    #: would have had this guard silently switched off for them. This repository's own rule
    #: is to scope a fix to observed behaviour, and five of those six were speculation.
    #: Exempting only what was measured leaves the guard on for everyone else, and makes the
    #: next mistake a failing test rather than a missed leak.
    _GENERIC_ACCOUNTS = frozenset({"root"})
    ...
    def test_the_exemption_covers_only_what_was_measured(self, monkeypatch):
        """An exemption list nothing bounds is how a privacy guard quietly retires.

        The first version exempted six names, five of them never observed anywhere. Review
        made the point that a membership test cannot tell a container default from a real
        person's account — so a developer called `build` would have had this switched off
        for them, invisibly. Two halves, because the set alone is not the claim:

        the set is exactly what was measured, and a generic-sounding name outside it is
        still checked, demonstrated on a constant that genuinely leaks it.
        """
        import os

        assert self._GENERIC_ACCOUNTS == frozenset({"root"}), (
            f"the exemption has grown beyond the measured case: {self._GENERIC_ACCOUNTS}. "
            "Add a name only with the command that observed it, as `root` has."
        )

        monkeypatch.setenv("USER", "build")
        monkeypatch.setattr(cs, "REASON_FAKE_LEAK", "window belongs to build", raising=False)
