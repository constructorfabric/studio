...
class TestCLIAgentsE2E(unittest.TestCase):
    ...
    def test_generate_agents_agent_opencode_form_matches_convenience_flag(self):
        """`--agent opencode` must behave identically to `--opencode` on the write path (PR #71 review comment).

        The only other generate-agents/opencode e2e test above uses the
        `--opencode` convenience flag exclusively; `--agent opencode` was
        previously exercised only against the read-only `agents` command.
        """
        with TemporaryDirectory() as td:
            root = Path(td) / "proj"
            _bootstrap_generator_project(root)
