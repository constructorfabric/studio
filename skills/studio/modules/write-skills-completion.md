...
DO:
  ...
  EMIT a completed SKILL_RESULT envelope with skill = cf-prompting-fix when REVIEW_FIXES_APPLIED == true and REVIEW_LOOP_REQUESTED == true, otherwise cf-prompting-gen, status = completed, produced_artifacts = skill-changes describing PATHS_WRITTEN when author output paths were captured or REVIEW_TARGET_PATHS when only fix target paths are available, report_outputs = [], missing_artifacts = [], assumptions = [], and suggested_next_skills = []
