...
DO:
  ...
  CONTINUE StudioInstructionsMemoryLoadFiles WHEN STUDIO_INSTRUCTIONS_MEMORY != loaded
RULES:
...
```pdsl
UNIT StudioInstructionsMemoryLoadFiles
PURPOSE: Load the generated and project-specific Studio instruction files needed for session memory.
DO:
  ...
  CONTINUE StudioInstructionsMemoryRemember
```

```pdsl
UNIT StudioInstructionsMemoryRemember
PURPOSE: Classify and remember the loaded Studio instruction files for the session.
DO:
