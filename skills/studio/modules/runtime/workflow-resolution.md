...
DO:
  RUN WorkflowResolutionCommandSetup
  RUN WorkflowResolutionEnumerate
...
```pdsl
UNIT WorkflowResolutionCommandSetup
PURPOSE: Resolve `{cfs_cmd}` before workflow discovery enumerates kit workflows.
DO:
...
```pdsl
UNIT WorkflowResolutionEnumerate
PURPOSE: Enumerate core and kit workflows and map them onto cf-* skill names.
DO:
