...
DO:
  ...
  WAIT user.reply
  STOP_TURN
  CONTINUE BrainstormTopicCaptureResume
RULES:
...
DO:
  ...
  WAIT user.reply
  STOP_TURN
  CONTINUE BrainstormOfferResume
RULES:
  ...
  NEVER offer, accept, or surface the `save` option when writes are not allowed or the destination is chat-only; pre-check write availability before rendering the offer
...
```pdsl
UNIT BrainstormOfferResume
PURPOSE: Route the resumed offer reply to the correct outcome unit.
DO:
