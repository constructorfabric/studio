...
DO:
  ...
  WHEN findings remain but no fixes were applied this iteration (none approved, none applicable, or the ReviewFixApprovalGate resolved to none):
    ...
    RUN NextActionsOffer with cf-documenting-fix as suggested
    STOP_TURN and report the remaining findings — re-reviewing unchanged content cannot change the result
