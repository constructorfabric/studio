...
WHEN:
  - REQUIRE context exhausted before every code_path is fully read
...
RULES:
  - NEVER emit a PASS verdict on partially-read targets
  - ALWAYS Orchestrator ALWAYS treat type=PARTIAL_CHECKPOINT as incomplete review coverage
  - NEVER collapse PARTIAL_CHECKPOINT into a clean validation report
```
...
DO:
  ...
  - RUN Walk every applicable category with status and line-numbered evidence
  - RUN Emit Findings for FAIL / PARTIAL categories only
```
...
RULES:
  ...
  - ALWAYS findings JSON ALWAYS be present in both output shapes
  - ALWAYS AP-001..AP-008 self-check ALWAYS be present
