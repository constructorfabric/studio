...
Files listed in `target_paths` or `cross_ref_paths` are reviewed resources:
...
DO:
  ...
  - REQUIRE full read of target_paths or declared cross_ref_paths cannot complete within available context budget:
    - EMIT PARTIAL_CHECKPOINT (see schema below)
    - STOP_TURN

  - REQUIRE checklist_required AND the synthesized final dispatch prompt is missing
  ...
  - RUN Read every declared cross_ref_path in full via Read tool (fresh read this turn)
     before any PASS outcome is allowed
  - RUN Walk EVERY checklist category individually
     Produce per-category status: PASS | FAIL | PARTIAL | N/A
     Include evidence: quoted line(s) and line numbers
  - RUN For each FAIL or PARTIAL category, emit one or more Findings
...
RULES:
  ...
  - ALWAYS set mechanical: false for:
...
RULES:
  ...
  - ALWAYS have review_result JSON block before the Validation Report block
  - ALWAYS have per-category status with evidence for every applicable checklist category
  - ALWAYS have findings JSON block (empty array when all categories PASS)
  - ALWAYS have non-empty mechanical_rationale on every finding object
  - ALWAYS fail closed when any finding omits mechanical_rationale:
    the response is incomplete, NEVER be surfaced as fix-ready output,
    and ALWAYS be rejected for re-run rather than patched by orchestrator substitution
