"""
ralphex Plan Export Compiler - Compile Constructor Studio plans into ralphex-compatible format.

Transforms Constructor Studio plan manifests and phase files into ralphex Markdown plans
...
@cpt-algo:cpt-studio-algo-ralphex-delegation-compile-plan:p1
@cpt-algo:cpt-studio-algo-ralphex-delegation-map-phase:p1
@cpt-flow:cpt-studio-flow-ralphex-delegation-handoff:p1
@cpt-state:cpt-studio-state-ralphex-delegation-lifecycle:p1
"""
...
def compile_delegation_plan(plan_dir: str) -> str:
    """Compile a Constructor Studio plan into a ralphex-compatible Markdown plan.
    ...
    Args:
        plan_dir: Path to the Constructor Studio plan directory containing ``plan.toml``
                  and phase files.
...
def map_phase_to_task(
    ...
    """Map a single Constructor Studio phase to a ralphex ``### Task N:`` block.
    ...
    Args:
        phase_content: Raw Markdown content of the Constructor Studio phase file.
...
def _resolve_plan_manifest_path(path_value: str, plan_dir: str) -> Path:
    ...
    if rooted.exists() or str(candidate).startswith((".bootstrap/", "studio/", ".plans/")):
        return rooted
...
def run_validation_commands(commands: list[str], cwd: Optional[str] = None) -> dict:
    """Re-run deterministic validation commands from the original Constructor Studio plan.
...
def report_handoff(
    ...
    Args:
        ...
        validation_passed: Whether Constructor Studio validation commands passed.
...
# @cpt-begin:cpt-studio-state-ralphex-delegation-lifecycle:p1
...
class DelegationLifecycle:
    ...
    # @cpt-end:inst-fail
# @cpt-end:cpt-studio-state-ralphex-delegation-lifecycle:p1
...
# @cpt-begin:cpt-studio-dod-ralphex-delegation-bootstrap:p1
def check_bootstrap_needed(repo_root: str) -> dict:
    ...
    return {
        ...
        "message": (
            ...
            "This requires your explicit approval — Constructor Studio will never run "
            "`ralphex --init` automatically."
        ),
    }
# @cpt-end:cpt-studio-dod-ralphex-delegation-bootstrap:p1
...
_REVIEW_OVERRIDE_BEGIN = "<!-- @cpt-begin:studio-review-override -->"
_REVIEW_OVERRIDE_END = "<!-- @cpt-end:studio-review-override -->"
_REVIEW_OVERRIDE_INTRO = (
    "Constructor Studio-managed final review step."
)
...
def _resolve_analyze_workflow_path(repo_root: Path) -> str:
    adapter_rel = _read_studio_var(repo_root)
    candidates: list[str] = []
    if adapter_rel:
        candidates.append(adapter_rel)
    for candidate in ("studio", ".cf", ".bootstrap", ".cpt"):
    ...
    raise OSError(
        "Could not resolve Constructor Studio install directory for final analyze step. "
        "Ensure root AGENTS.md defines `cf-path` or install Constructor Studio in a standard adapter directory."
    )
