...
def _find_adapter_path(entry: Path) -> Optional[str]:
    ...
    # v3 discovery: read studio_path variable from AGENTS.md
    studio_rel = _read_studio_var(entry)
    if studio_rel:
        candidate = (entry / studio_rel).resolve()
        if candidate.is_dir() and (candidate / "config").is_dir():
            return studio_rel

    # Fallback: try find_studio_directory for recursive search
    found_dir = find_studio_directory(entry)
...
# @cpt-algo:cpt-studio-algo-workspace-infer-role:p1
def _infer_role(repo_path: Path) -> str:
    ...
    if capabilities > 1:
        return "full"
    # @cpt-end:cpt-studio-algo-workspace-infer-role:p1:inst-role-if-multi
    # @cpt-begin:cpt-studio-algo-workspace-infer-role:p1:inst-role-if-kits
    if has_kits:
        return "kits"
    # @cpt-end:cpt-studio-algo-workspace-infer-role:p1:inst-role-if-kits
    # @cpt-begin:cpt-studio-algo-workspace-infer-role:p1:inst-role-if-artifacts
    if has_docs:
        return "artifacts"
    # @cpt-end:cpt-studio-algo-workspace-infer-role:p1:inst-role-if-artifacts
    # @cpt-begin:cpt-studio-algo-workspace-infer-role:p1:inst-role-if-codebase
    if has_src:
        return "codebase"
    # @cpt-end:cpt-studio-algo-workspace-infer-role:p1:inst-role-if-codebase
    # @cpt-begin:cpt-studio-algo-workspace-infer-role:p1:inst-role-return-full
    return "full"
    # @cpt-end:cpt-studio-algo-workspace-infer-role:p1:inst-role-return-full
...
# @cpt-algo:cpt-studio-algo-workspace-discover-nested:p1
def _scan_nested_repos(
    ...
    # @cpt-begin:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-check-depth
    if _current_depth > max_depth:
        return discovered
    # @cpt-end:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-check-depth
    # @cpt-begin:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-list-entries
    try:
    ...
    # @cpt-begin:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-foreach-entry
    for entry in entries:
        if not entry.is_dir() or entry.name.startswith(".") or entry.is_symlink():
            continue
        # @cpt-end:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-foreach-entry
        # @cpt-begin:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-check-project
        if not _is_project_dir(entry):
            # @cpt-end:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-check-project
            # @cpt-begin:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-if-not-project
        ...
        discovered[key] = info
        # @cpt-end:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-add-source

    # @cpt-begin:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-return
    return discovered
    # @cpt-end:cpt-studio-algo-workspace-discover-nested:p1:inst-disc-return


# @cpt-begin:cpt-studio-state-workspace-config-lifecycle:p1:inst-config-create-inline
def _write_inline(
    project_root: Path,
    workspace_data: dict,
) -> Tuple[int, dict]:
    """Write workspace config inline into core.toml. Returns (exit_code, result_dict)."""
    # @cpt-end:cpt-studio-state-workspace-config-lifecycle:p1:inst-config-create-inline
    # @cpt-begin:cpt-studio-flow-workspace-init:p1:inst-write-inline-impl
    ...
    # @cpt-end:cpt-studio-flow-workspace-init:p1:inst-write-inline-impl


# @cpt-begin:cpt-studio-state-workspace-config-lifecycle:p1:inst-config-create-standalone
def _write_standalone(
    output_path: Path,
    workspace_data: dict,
) -> Tuple[int, dict]:
    """Write standalone .cf-workspace.toml. Returns (exit_code, result_dict)."""
    # @cpt-end:cpt-studio-state-workspace-config-lifecycle:p1:inst-config-create-standalone
    # @cpt-begin:cpt-studio-flow-workspace-init:p1:inst-write-standalone-impl
    ...
    # @cpt-end:cpt-studio-flow-workspace-init:p1:inst-write-standalone-impl


# @cpt-begin:cpt-studio-flow-workspace-init:p1:inst-init-helpers
...
def _write_workspace_config(
    ...
    # @cpt-begin:cpt-studio-flow-workspace-init:p1:inst-if-inline
...
def cmd_workspace_init(argv: List[str]) -> int:
    ...
    p.add_argument(
        "--output", default=None,
        help="Where to write .cf-workspace.toml (default: scan root)",
    )
    p.add_argument(
    ...
    # @cpt-begin:cpt-studio-flow-workspace-init:p1:inst-determine-scan-root
    scan_root = Path(args.root).resolve() if args.root else project_root
    # @cpt-end:cpt-studio-flow-workspace-init:p1:inst-determine-scan-root
    # @cpt-begin:cpt-studio-flow-workspace-init:p1:inst-if-no-scan-root
    if not scan_root.is_dir():
        ui.result({"status": "ERROR", "message": f"Scan root directory not found: {scan_root}"})
        return 1
    # @cpt-end:cpt-studio-flow-workspace-init:p1:inst-if-no-scan-root
    ...
    # @cpt-end:cpt-studio-flow-workspace-init:p1:inst-build-workspace-data

    # @cpt-begin:cpt-studio-flow-workspace-init:p1:inst-if-dry-run
    if args.dry_run:
        ui.result({
            "status": "DRY_RUN",
        ...
        return 0
    # @cpt-end:cpt-studio-flow-workspace-init:p1:inst-if-dry-run
    ...
    return exit_code
    # @cpt-end:cpt-studio-flow-workspace-init:p1:inst-return-init-ok
...
# @cpt-begin:cpt-studio-flow-workspace-init:p1:inst-init-human-fmt
