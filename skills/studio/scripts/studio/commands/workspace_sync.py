...
# @cpt-flow:cpt-studio-flow-workspace-sync:p1
...
def _collect_git_sources(ws_cfg, source_name):
    ...
    # @cpt-begin:cpt-studio-flow-workspace-sync:p1:inst-sync-if-source-not-found
    if source_name is not None:
        ...
        # @cpt-end:cpt-studio-flow-workspace-sync:p1:inst-sync-if-source-not-found
        # @cpt-begin:cpt-studio-flow-workspace-sync:p1:inst-sync-if-no-url
        if not src_entry.url:
            return None, {
                "status": "ERROR",
                "message": f"Source '{source_name}' has no Git URL — only Git URL sources can be synced",
            }
        # @cpt-end:cpt-studio-flow-workspace-sync:p1:inst-sync-if-no-url
...
def cmd_workspace_sync(argv: List[str]) -> int:
    ...
    if src_err is not None:
        ui.result(src_err)
        return 1
    # @cpt-end:cpt-studio-flow-workspace-sync:p1:inst-sync-collect-sources

    # @cpt-begin:cpt-studio-flow-workspace-sync:p1:inst-sync-if-no-git-sources
    if not git_sources:
        ui.result({
            "status": "OK",
        ...
        return 0
    # @cpt-end:cpt-studio-flow-workspace-sync:p1:inst-sync-if-no-git-sources
...
# @cpt-begin:cpt-studio-flow-workspace-sync:p1:inst-sync-human-fmt
def _human_workspace_sync(data: dict) -> None:
    ...
    ui.blank()
# @cpt-end:cpt-studio-flow-workspace-sync:p1:inst-sync-human-fmt
