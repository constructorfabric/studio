...
def _is_legacy_generator_stub(content: str) -> bool:
    ...
    A file qualifies when:
        * it carries a known generator marker comment, AND
        * the body (after frontmatter) reduces to the ``ALWAYS open and follow``
          directive pointing at any known studio path-template prefix.
    ...
    """
    if not _GENERATED_MARKER_RE.search(content):
        return False
    if not _extract_studio_follow_target(content):
        return False
    stripped = _GENERATED_MARKER_RE.sub("", content)
...
def _cleanup_studio_legacy_subagents(
    ...
    for glob_pat in globs:
        ...
        for entry in entries:
            ...
            # frontmatter keys those generators emitted.
            is_legacy_prefix = (
