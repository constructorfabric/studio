...
def _resolve_config_path(project_root: Path, studio_rel: str) -> Path:
    """Resolve the config file path, preferring config/ subdirectory."""
    config_path = (project_root / studio_rel / "config" / _CONFIG_FILENAME).resolve()
...
class SourceEntry:
    ...
    def to_dict(self) -> dict:
        ...
        if self.role != "full":
            d["role"] = self.role
        return d
# @cpt-end:cpt-studio-algo-workspace-resolve-source:p1:inst-resolve-datamodel
...
# @cpt-begin:cpt-studio-algo-workspace-find-config:p1:inst-find-config-datamodel
@dataclass
class WorkspaceConfig:
    ...
    def load(cls, workspace_path: Path) -> Tuple[Optional["WorkspaceConfig"], Optional[str]]:
        ...
        Args:
            workspace_path: Absolute path to .cf-workspace.toml
    ...
    # @cpt-algo:cpt-studio-algo-workspace-resolve-source:p1
    def resolve_source_path(self, source_name: str) -> Optional[Path]:
        ...
        # @cpt-end:cpt-studio-algo-workspace-resolve-source:p1:inst-resolve-return
        # @cpt-begin:cpt-studio-algo-workspace-resolve-source:p1:inst-resolve-git-url-delegate
        # @cpt-begin:cpt-studio-algo-workspace-resolve-git-url:p1:inst-resolve-git-url-delegate
        if src.url:
            from .git_utils import resolve_git_source
            return resolve_git_source(src, self.resolve or ResolveConfig(), base)
        # @cpt-end:cpt-studio-algo-workspace-resolve-git-url:p1:inst-resolve-git-url-delegate
        # @cpt-end:cpt-studio-algo-workspace-resolve-source:p1:inst-resolve-git-url-delegate
...
def find_workspace_config(project_root: Path) -> Tuple[Optional[WorkspaceConfig], Optional[str]]:
    ...
    if cfg is None:
        ...
        studio_rel = _read_studio_var(project_root)
        if studio_rel:
            candidate = _resolve_config_path(project_root, studio_rel)
            if candidate.is_file():
                return None, f"Failed to parse config: {candidate}"
        return _find_standalone_workspace(project_root)
    # @cpt-end:cpt-studio-algo-workspace-find-config:p1:inst-find-if-no-config
...
# @cpt-begin:cpt-studio-state-workspace-config-lifecycle:p1:inst-config-load-inline-impl
def load_inline_config(project_root: Path) -> Tuple[Optional[Path], dict, Optional[str]]:
    ...
    studio_rel = _read_studio_var(project_root)
    if not studio_rel:
        return None, {}, "No cf-path found in AGENTS.md. Run 'cfs init' first."

    config_path = _resolve_config_path(project_root, studio_rel)
...
# @cpt-end:cpt-studio-state-workspace-config-lifecycle:p1:inst-config-load-inline-impl
