...
def _menu_name(rest: str) -> Optional[str]:
    ...
    Non-ASCII names are **not** handled and that is deliberate. The validator rejects
    `CaféMenu` outright and silently truncates its decomposed spelling to `Cafe` -- so
    reading the same name as `Cafe` here agrees with the validator, which is the only
    agreement that matters: a name this walk read differently would be a name no gate can
    ever emit. Zero occurrences in the pinned kit, and the fail-safe direction is to match
    the validator rather than to out-guess it.
    """
    token = rest.split(None, 1)[0] if rest.split() else ""
    if not token[:1].isalpha():
        return None
    cut = 1
    while cut < len(token) and (token[cut].isalnum() or token[cut] in "_-"):
        cut += 1
    return token[:cut]
...
def _said(value: object) -> str:
    ...
    # The fallback still collapses `$HOME`. Returning the raw path truncated would make
    # the one failure path of a username guard the one that leaks the username -- the same
    # miss, in the same shape, that the reversal check's fallback had.
    text = str(value)
    try:
        home = str(Path.home())
    except (OSError, RuntimeError) as exc:
        # Both layers gone. The guard cannot know which prefix to remove, so it stops
        # reporting a path at all rather than reporting an absolute one — which is what
        # the last version did, returning the raw path truncated and putting the username
        # in the field the whole function exists to keep it out of. Raised in review: the
        # fallback's own fallback was the one that leaked.
        logger.warning("gate surface: neither the shared redactor nor the home directory "
                       "could be read, so only the final path component is reported: %s",
                       type(exc).__name__)
        return ".../" + PurePosixPath(text.replace("\\", "/")).name
    if text.startswith(home):
        text = "~" + text[len(home):]
    return text[:_MAX_REPORTED]
...
def _halts_outside_menus(text: str) -> int:
    ...
    for line in text.splitlines():
        ...
        if section not in _ACTION_SECTIONS:
            continue
        if action.startswith(_EMIT_MENU):
            emitted = True
