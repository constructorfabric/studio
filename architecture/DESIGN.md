...
| `warnings` | array of entries, each carrying `level` (`warning` or `error`) and `reason` (fixed-enum text when `level: warning`; the underlying OS error text when `level: error`) | omitted, unless the harness's final state-file persistence step failed (see note below) | present, ≥ 1 entry, `level: warning`, fixed-enum `reason` | omitted, unless the persistence step failed (see note below) | present, ≥ 1 entry, `level: error`, `reason` is the underlying OS error text | omitted |
