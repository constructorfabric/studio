...
significant_changes:
  - version: 1.7.0
    date: 2026-09-25
    summary: Round-9 hook-based session routing review fixes — `HarnessRoutingState` now explicitly records that the execution receipt, like the per-harness state file, is gitignored and per-machine (never committed), and documents the accepted fresh-clone consequence (a clone re-enters the pending-first-run window until its own client fires the hook once); `HarnessRoutingOutcome`'s render-summary algorithm now iterates every entry of a `warnings` array independently of `routing_mode`, so a `file` outcome carrying both a warning-level fallback reason and an error-level persist-failure entry (a failed `hook` promotion) renders both, not just one.
  - version: 1.6.0
    date: 2026-09-24
    summary: Round-6 hook-based session routing review fixes — `HarnessRoutingState` now records that the per-harness state file has a second reader (the harness's own session-start hook command, which reads `routing_mode` on every firing and treats a missing/unparseable/unrecognised-`schema_version` file as not `hook`) and that writes are therefore atomic via same-directory temp file plus rename, and the `HarnessRoutingOutcome` persistence-failure exception is broadened from "promoted from `file` to `hook`" to any harness newly promoted to `hook` whose state file does not already durably record `hook` (including from `off`, from `errored`, or from an absent/corrupt file), while a harness already recording `hook` on disk stays `hook` on a bookkeeping-only write failure.
  - version: 1.5.0
    date: 2026-09-24
    summary: Round-4 hook-based session routing review fix — added an explicit field-level schema (type, presence rule per `routing_mode` value) for `HarnessRoutingOutcome`, clarifying that `hook_path`/`file_marker_paths` are omitted (never `null`) outside their applicable mode, that `warnings` is omitted rather than emitted empty, and that a state-file persistence failure is reported as an independent `level: error` `warnings` entry without downgrading `routing_mode`.
  - version: 1.4.0
    date: 2026-09-23
    summary: Aligned the routing entities with the hook-based session routing FEATURE review — `HarnessRoutingOutcome.routing_mode` now enumerates the report-only `unknown` value alongside `off`/`file`/`hook`/`errored`, and `HarnessRoutingState` records that `Off` is persisted rather than deleted on disablement and that a persisted `errored` is re-tested against installed resources on inspection.
  - version: 1.3.0
    date: 2026-09-16
...
| HarnessRoutingState | Per-harness routing delivery state — exactly one of `Off`, `FileFallback`, `HookInstalled`, `Errored` (serialized as `routing_mode` `off`/`file`/`hook`/`errored`) — persisted per harness for every mode, including `Off` (disablement rewrites the state file to `off` rather than deleting it), and independently of the `AGENTS.md`/`CLAUDE.md` marker scan. Derived from installed resources on inspection, with the persisted file used only as a cross-check: precedence is `HookInstalled` (verified Studio-owned hook entry plus confirming execution receipt) > `FileFallback` (shared managed block present) > `Off`; `unknown` is reported only when persisted and derived state disagree, never merely because a file is absent. `Errored` is the one state derivation cannot produce, so it has its own inspection rule: a persisted `errored` is re-tested against the resources installed now — if they resolve cleanly to `off`/`file`/`hook` the freshly derived state is reported (the error was transient or since repaired), otherwise `errored` is reported again with the reason recorded in the state file. The state file has a **second reader** besides the CLI: the harness's own session-start hook command reads its `routing_mode` on every firing to decide whether to emit the payload or only stamp its receipt, treating a missing, unparseable, or unrecognised-`schema_version` file as absent and therefore not `hook`; writes are consequently atomic (temp file in the same directory, renamed into place) so a concurrent hook read never observes a partial write. Both the state file and the execution receipt it cross-checks are **gitignored, per-machine** artifacts (covered by the same managed `.gitignore` block, never committed) — proof-of-installation for *this* machine, not project configuration — while the hook entry that names the receipt lives in the harness's own (tracked) config; a fresh clone of an already-`HookInstalled` project therefore inherits the tracked hook entry but not the local receipt, and correctly re-derives `FileFallback` until that developer's own client fires the hook once, an accepted instance of the already-documented pending-first-run window, not a new case | Per-harness state file `<harness-config-dir>/.cf-studio-routing-state.json` (`.claude/`, `.codex/`, `.cursor/`, `.github/`), carrying `schema_version`, gitignored; per-harness execution receipt (same config directory, gitignored); harness-native hook config (`.claude/settings.json`, `.codex/hooks.json`, `.cursor/hooks.json`, `.github/hooks/<name>.json`), tracked; shared root `AGENTS.md`/`CLAUDE.md` managed block |
| HarnessRoutingOutcome | Result of compiling routing delivery for one harness during a run: `routing_mode` (`off`/`file`/`hook`/`errored`/`unknown` — the four serialized `HarnessRoutingState` values plus `unknown`, a report-only value emitted by inspection when persisted and derived state disagree and never persisted as a state), the hook config path when a hook entry is installed or the **pair** of shared file-marker paths (root `AGENTS.md` and root `CLAUDE.md`, always reported together) when the file fallback is in effect — all project-root-relative — plus any fallback warning with its fixed-enum reason, or the underlying OS error when `errored`. Field-level presence, type, and cardinality per `routing_mode` value are defined in the field schema below | Produced by `cfs generate-agents`; reported under a new top-level `routing` key in the install-outcome summary (human table and `--json`) alongside those commands' existing fields, and re-derived by `cfs agents` |
...
**`HarnessRoutingOutcome` field schema**: presence, type, and cardinality are defined per `routing_mode` value below. No field is ever emitted as JSON `null`; a field not marked "present" for a given mode is omitted from the record entirely.

| Field | Type | `off` | `file` | `hook` | `errored` | `unknown` |
|---|---|---|---|---|---|---|
| `routing_mode` | string, one of `off`/`file`/`hook`/`errored`/`unknown` | present | present | present | present | present |
| `hook_path` | string, project-root-relative | omitted | omitted | present | omitted | omitted |
| `file_marker_paths` | array of exactly 2 strings, project-root-relative (root `AGENTS.md` then root `CLAUDE.md`) | omitted | present | omitted | omitted | omitted |
| `warnings` | array of entries, each carrying `level` (`warning` or `error`) and `reason` (fixed-enum text when `level: warning`; the underlying OS error text when `level: error`) | omitted, unless the harness's final state-file persistence step failed (see note below) | present, ≥ 1 entry with a fixed-enum `level: warning` reason; also includes a `level: error` entry with the underlying OS error text when this `file` outcome is a failed `hook` promotion (see note below) | omitted, unless the persistence step failed (see note below) | present, ≥ 1 entry, `level: error`, `reason` is the underlying OS error text | omitted |
| `persisted_mode` | string, one of the four serialized `HarnessRoutingState` values | omitted | omitted | omitted | omitted | present |
| `derived_mode` | string, one of the four serialized `HarnessRoutingState` values | omitted | omitted | omitted | omitted | present |

The `warnings` key itself is omitted rather than emitted as an empty array whenever there is nothing to report for that harness, matching the existing omit-when-empty presence convention already used elsewhere in the CLI's JSON output — specifically `workspace-info`'s `config_warnings` field (`specs/cli.md`: `string[]?`, "present only when warnings exist"), not `cfs info --json`, whose documented output carries no `warnings`-like field at all. Only the omit-when-empty **presence rule** is reused from that precedent; the **element shape** is a deliberate divergence: `workspace-info`'s own top-level `warnings` field, and its `config_warnings` field, are both a flat `string[]` of message text, while `HarnessRoutingOutcome.warnings` introduces a new `{level, reason}` object-array shape so a machine consumer can distinguish an advisory fallback notice (`level: warning`) from a surfaced write failure (`level: error`) without string-matching. A `warnings` entry is independent of `routing_mode`: the channel-establishment outcome (`off`/`file`/`hook`/`errored`) and the outcome of persisting that mode to the harness's own state file are reported separately. A harness whose hook or fallback delivery succeeded but whose final state-file write failed still reports its correct `routing_mode` (`off`, `file`, or `hook`) plus one `level: error` `warnings` entry — it is not downgraded to `errored` for a bookkeeping write failure that did not affect delivery. The one exception is a harness being **newly promoted** to `hook` this run — one whose state file does not already durably record `hook`, whether its persisted mode was `file`, `off`, or `errored`, or its state file was absent, unparseable, or of unrecognised `schema_version`: because emission over the hook channel is gated on the persisted mode the hook command reads at execution time actually saying `hook` (`cpt-studio-dod-hook-based-session-routing-payload-transport`'s receipt-only rule), a persist failure at the exact moment of promotion means the hook channel is not yet live either, so this harness's `routing_mode` is reported as `file`, not `hook`, for this run — see `cpt-studio-algo-hook-based-session-routing-reconcile-marker`'s promotion-before-removal ordering in `hook-based-session-routing.md`. A harness that **already** recorded `hook` on disk from a prior run is not in this exception: its state file already carries the value the hook command reads, so a failed rewrite is bookkeeping-only and `routing_mode` stays `hook`.

**Fixed-enum `warnings.reason` values** (fallback warnings, `level: warning`, per `cpt-studio-dod-hook-based-session-routing-reason-enum`): the enum has exactly three members, each corresponding to a distinct `target_mode = file` return in `cpt-studio-algo-hook-based-session-routing-compile-harness`:

| Value | Meaning | Compile-harness step |
|---|---|---|
| `hook-unavailable` | The installed client does not currently expose a usable session-start hook — no hook event at all, an outdated client, or a project that has explicitly disabled hooks | `inst-return-file-no-hook` |
| `verification-failed` | A hook entry was written this run and then removed because it failed verification (`cpt-studio-algo-hook-based-session-routing-verify-hook`) | `inst-return-file-verify-failed` |
| `pending-first-run` | A verified hook entry exists but its confirming execution receipt has not yet been observed | `inst-return-file-pending-receipt` |

This resolves `hook-based-session-routing.md` Section 7 item (c) for the reasons that are structurally knowable ahead of implementation. It is deliberately **not** a complete resolution: whether `hook-unavailable` is further subdivided into separate values for "outdated client" versus "hooks explicitly disabled" remains open, tracked by that document's item (d), because it depends on an implementation-time decision about whether Studio probes for the specific cause or reports the single generic value shown above. `level: error` entries are never drawn from this enum — their `reason` is always the underlying OS error text from a filesystem failure (hook-entry write/removal, shared-marker write/removal, or state-file persistence), which cannot be enumerated ahead of time and remains free text.
