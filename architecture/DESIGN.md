...
| HarnessRoutingState | Per-harness routing delivery state — exactly one of `Off`, `FileFallback`, `HookInstalled` — persisted per harness independently of the `AGENTS.md`/`CLAUDE.md` marker scan, and re-derived rather than replayed on inspection | Per-harness routing state file, harness-native hook config (e.g. `.claude/settings.json`), shared root `AGENTS.md`/`CLAUDE.md` managed block |
| HarnessRoutingOutcome | Result of compiling routing delivery for one harness during a run: routing mode, the hook path or shared file-marker path (project-root-relative), and any fallback warning with its fixed-enum reason | Produced by `cfs generate-agents`; reported in the install-outcome summary (human table and `--json`) and re-derived by `cfs agents` |
