...
**Location**: `{cf-constructor-path}/config/kits/<slug>/artifacts/<KIND>/rules.md`
...
1. Find system for target artifact in `{cf-constructor-path}/config/artifacts.toml`
2. Get `kit` ID from system (e.g., `"cypilot-sdlc"`)
3. Resolve kit slug from `{cf-constructor-path}/config/core.toml` (e.g., `"sdlc"`)
4. Build full path: `{cf-constructor-path}/config/kits/{slug}/artifacts/{KIND}/rules.md`
...
Format: `` `path` — description ``. All paths are relative to the same kit config directory (`{cf-constructor-path}/config/kits/{slug}/`).

**taxonomy.md** is an optional dependency — generated by `cpt generate-resources` at `{cf-constructor-path}/config/kits/{slug}/taxonomy.md`. It provides artifact kind definitions, ID type descriptions, and cross-artifact traceability context. Workflows MAY use it to understand the kit's naming conventions and artifact relationships.
...
1. DETECT artifact type (explicit parameter or from artifacts.toml)
   ↓
2. RESOLVE kit:
   - Find system in {cf-constructor-path}/config/artifacts.toml
...
For a PRD artifact, `{cf-constructor-path}/config/kits/sdlc/artifacts/PRD/rules.md`:
...
|---|-------|----------|---------------|
| S.1 | rules.md exists at correct path | YES | File at `{cf-constructor-path}/config/kits/<slug>/artifacts/<KIND>/rules.md` |
...
- **Kit specification**: `{cf-constructor-path}/.core/architecture/specs/kit/kit.md` — kit structure, installation, update model
- **Execution protocol**: `{cf-constructor-path}/.core/requirements/execution-protocol.md` — workflow execution rules
- **Identifiers & Traceability**: `{cf-constructor-path}/.core/architecture/specs/traceability.md` — ID formats, naming, code traceability
- **CLI**: `{cf-constructor-path}/.core/architecture/specs/cli.md` — `generate`, `validate` commands
