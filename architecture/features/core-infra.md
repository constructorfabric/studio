...
6. [x] - `p1` - Restrict the log, its lock and their directory to the owner on creation, so a log this module calls private is not left world-readable by the ambient umask - `inst-log-restrict-perms`
   - [x] - `p1` - Link a rotation to the segment it created, by opening the new live segment with a `rotate` event naming its predecessor, and read a `.1` segment only when the live segment's *first* event claims it: nothing in the filesystem distinguishes a backup this log rotated into from one left behind when an operator cleared the live log, and reading it unconditionally presented events they meant to be gone as continuous history. An unclaimed segment is excluded and warned about rather than dropped in silence, since a log rotated before the link existed is indistinguishable from a cleared one and only the reader knows which it has - `inst-log-rotation-link`
