...
- [x] `p1` - **ID**: `cpt-studio-featstatus-workspace`

## 1. Feature Context

- [x] `p1` - `cpt-studio-feature-workspace`
...
Enables teams working across multiple repositories to federate their Studio-managed artifacts — discovering repos in nested sub-directories, resolving cross-repo artifact paths, and collecting IDs across workspace boundaries — without merging adapters or modifying individual project configs. Addresses PRD requirement for workspace federation (`cpt-studio-fr-core-workspace`) and extends the core traceability system (`cpt-studio-fr-core-traceability`) with cross-repo resolution.

**Delegation context note**: When Studio delegates execution to an external executor (e.g., ralphex), the delegated execution uses the same workspace/source-resolved context as the primary Studio session. Workspace source paths, adapter bindings, and cross-repo artifact resolution remain authoritative for delegated tasks. This feature does not own delegation logic — that belongs to `cpt-studio-feature-ralphex-delegation` — but it provides the context foundation that delegation consumers rely on (see `cpt-studio-adr-ralphex-delegation-skill`).

### 3. Actors

| Actor | Role in Feature |
|-------|-----------------|
| `cpt-studio-actor-user` | Runs `workspace-init`, `workspace-add` (with `--inline` flag for inline mode), `workspace-info`, `workspace-sync`; uses `validate --local-only` and `list-ids --source` |
| `cpt-studio-actor-studio-cli` | Discovers workspace config during context loading, resolves cross-repo paths, loads source contexts |

### 4. References

- **PRD**: [PRD.md](../PRD.md) — `cpt-studio-fr-core-workspace`, `cpt-studio-fr-core-traceability`, `cpt-studio-fr-core-workspace-git-sources`, `cpt-studio-fr-core-workspace-cross-repo-editing`
- **Design**: [DESIGN.md](../DESIGN.md) — `cpt-studio-component-config-manager`, `cpt-studio-component-traceability-engine`
- **Design Principles**: `cpt-studio-principle-traceability-by-design`, `cpt-studio-principle-determinism-first`, `cpt-studio-principle-zero-harm`
- **Design Constraints**: `cpt-studio-constraint-python-stdlib`
- **CLI Spec**: [cli.md](../specs/cli.md)
- **Dependencies**: `cpt-studio-feature-core-infra`, `cpt-studio-feature-traceability-validation`
...
- [x] `p1` - **ID**: `cpt-studio-flow-workspace-init`
...
**Steps**:
1. [x] - `p1` - User invokes `cfs workspace-init [--root DIR] [--output PATH] [--inline] [--force] [--max-depth N] [--dry-run]` - `inst-user-workspace-init`
...
6. [x] - `p1` - Algorithm: discover repos in nested sub-directories using `cpt-studio-algo-workspace-discover-nested` (limited to `--max-depth` levels, default 3) - `inst-discover-nested`
...
11. [x] - `p1` - **ELSE** write standalone `.studio-workspace.toml` - `inst-else-standalone`
...
14. [x] - `p1` - Implementation: write standalone `.studio-workspace.toml` file - `inst-write-standalone-impl`
...
- [x] `p1` - **ID**: `cpt-studio-flow-workspace-add`
...
5. [x] - `p1` - **ELSE** find existing workspace config using `cpt-studio-algo-workspace-find-config` - `inst-add-find-ws`
...
- [x] `p1` - **ID**: `cpt-studio-flow-workspace-info`
...
**Steps**:
1. [x] - `p1` - User invokes `cfs workspace-info` - `inst-user-workspace-info`
2. [x] - `p1` - Find project root - `inst-info-find-root`
3. [x] - `p1` - **IF** no project root **RETURN** error (exit 1) - `inst-info-if-no-root`
4. [x] - `p1` - Find workspace config using `cpt-studio-algo-workspace-find-config` - `inst-info-find-ws`
...
- [x] `p1` - **ID**: `cpt-studio-flow-workspace-sync`
...
**Steps**:
1. [x] - `p1` - User invokes `cfs workspace-sync [--source NAME] [--dry-run]` - `inst-user-workspace-sync`
...
4. [x] - `p1` - Find workspace config using `cpt-studio-algo-workspace-find-config` - `inst-sync-find-ws`
...
- [x] `p1` - **ID**: `cpt-studio-algo-workspace-find-config`
...
**Steps**:
1. [x] - `p1` - Load project config from `core.toml` (via AGENTS.md studio_path) - `inst-find-load-project-config`
...
5. [x] - `p1` - **IF** `workspace` key absent or config not loaded: check for standalone `.studio-workspace.toml` at project root; **IF** file exists load and **RETURN** `(WorkspaceConfig, None)` or `(None, error)`; **ELSE RETURN** `(None, None)` — no workspace, single-repo mode - `inst-find-return-none`
...
8. [x] - `p1` - Implementation: discover standalone `.studio-workspace.toml` at project root - `inst-find-standalone-impl`

### Discover Nested Repos

- [x] `p1` - **ID**: `cpt-studio-algo-workspace-discover-nested`
...
3. [x] - `p1` - **FOR EACH** directory entry (skip hidden dirs and symlinks) - `inst-disc-foreach-entry`
   ...
   3. [x] - `p1` - Find adapter path: read `studio_path` from AGENTS.md, fallback to `find_studio_directory` - `inst-disc-find-adapter`
   ...
   6. [x] - `p1` - Algorithm: infer role using `cpt-studio-algo-workspace-infer-role` - `inst-disc-infer-role`
...
- [x] `p1` - **ID**: `cpt-studio-algo-workspace-resolve-source`
...
5. [x] - `p1` - **IF** `source.url` is set, delegate to `cpt-studio-algo-workspace-resolve-git-url` - `inst-resolve-git-url-delegate`
...
- [x] `p1` - **ID**: `cpt-studio-algo-workspace-infer-role`
...
- [x] `p1` - **ID**: `cpt-studio-algo-workspace-load-context`

**Input**: Primary StudioContext

**Output**: WorkspaceContext or None (if no workspace config found)

**Steps**:
1. [x] - `p1` - Find workspace config using `cpt-studio-algo-workspace-find-config` - `inst-ctx-find-config`
...
- [x] `p1` - **ID**: `cpt-studio-algo-workspace-resolve-artifact`
...
- [x] `p1` - **ID**: `cpt-studio-algo-workspace-collect-ids`
...
- [x] `p1` - **ID**: `cpt-studio-algo-workspace-resolve-git-url`
...
- [x] `p1` - **ID**: `cpt-studio-algo-workspace-sync-git-source`
...
**Steps**:
1. [x] - `p1` - Resolve local path using `cpt-studio-algo-workspace-resolve-git-url` (clone-only, no fetch) - `inst-sync-resolve-path`
...
- [x] `p1` - **ID**: `cpt-studio-algo-workspace-resolve-adapter-context`

**Input**: `SourceContext` (reachable, with `adapter_dir` set)

**Output**: `StudioContext` loaded from the source's adapter, or `None`
...
5. [x] - `p1` - Load `StudioContext` from the adapter path (config, kits, templates, rules, constraints) - `inst-adapter-load-context`
...
8. [x] - `p1` - **RETURN** loaded `StudioContext` - `inst-adapter-return`
...
- [x] `p1` - **ID**: `cpt-studio-algo-workspace-determine-target`

**Input**: Target file path, `WorkspaceContext`

**Output**: `(SourceContext, StudioContext)` for the source owning the target file, or `(None, primary_context)` if the file belongs to the primary repo
...
2. [x] - `p1` - **FOR EACH** source in `workspace.sources` (ordered by path length descending for longest-prefix match) - `inst-target-foreach-source`
   1. [x] - `p1` - **IF** target path starts with source absolute path - `inst-target-if-match`
      1. [x] - `p1` - Resolve adapter context using `cpt-studio-algo-workspace-resolve-adapter-context` - `inst-target-resolve-adapter`
...
- [x] `p1` - **ID**: `cpt-studio-state-workspace-source-reachability`
...
- [x] `p1` - **ID**: `cpt-studio-state-workspace-config-lifecycle`
...
**Transitions**:
1. [x] - `p1` - **FROM** NO_WORKSPACE **TO** STANDALONE **WHEN** `workspace-init` creates `.studio-workspace.toml` - `inst-config-create-standalone`
...
- [x] `p1` - **ID**: `cpt-studio-dod-workspace-source-mgmt`
...
**Implements**:
- `cpt-studio-flow-workspace-add`
- `cpt-studio-algo-workspace-resolve-source`
...
- [x] `p1` - **ID**: `cpt-studio-dod-workspace-graceful-degradation`
...
- [x] `p1` - **ID**: `cpt-studio-dod-workspace-backward-compat`

The system **MUST** operate identically for projects without workspace configuration. Context loading **MUST** return `StudioContext` (not `WorkspaceContext`) when no workspace is found. All existing CLI commands **MUST** function without modification in single-repo mode. No workspace-related warnings **MUST** appear for non-workspace projects.
...
- [x] `p1` - **ID**: `cpt-studio-dod-workspace-sync`
...
**Implements**:
- `cpt-studio-flow-workspace-sync`
- `cpt-studio-algo-workspace-sync-git-source`
...
- [x] `workspace-init` scans nested sub-directories and generates `.studio-workspace.toml` with discovered sources
...
- [x] Workspace config discovery checks `core.toml` `workspace` key first, then falls back to standalone `.studio-workspace.toml` at project root (no parent directory walk-up)
