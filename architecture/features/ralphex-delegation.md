...
- [ ] `p1` - **ID**: `cpt-studio-featstatus-ralphex-delegation`

## 1. Feature Context

- [ ] `p1` - `cpt-studio-feature-ralphex-delegation`

### 1.1 Overview

Dedicated delegation skill that integrates Studio with ralphex (`https://ralphex.com/`), an external standalone terminal CLI for autonomous plan execution with Claude Code. The skill owns the entire ralphex-aware integration surface: executable discovery, availability diagnostics, optional project bootstrap, bounded plan export from canonical Studio sources, delegation invocation with mode flags, and post-run handoff with status and validation continuity. Studio remains the source of truth for SDLC guidance, decomposition, and validation contracts; ralphex becomes the optional autonomous execution backend for compiled plans.

### 1.2 Purpose

Without this feature, users must manually copy Studio plans and rules into ralphex format, breaking determinism and risking stale or incomplete SDLC context. This feature provides a first-class "delegate to ralphex" path that compiles Studio planning output into ralphex-compatible artifacts and manages the full delegation lifecycle.

**Requirements**: `cpt-studio-fr-core-workflows`, `cpt-studio-fr-core-execution-plans`, `cpt-studio-fr-core-agents`

**Principles**: `cpt-studio-principle-determinism-first`, `cpt-studio-principle-skill-documented`, `cpt-studio-principle-occams-razor`

### 1.3 Actors

| Actor | Role in Feature |
|-------|-----------------|
| `cpt-studio-actor-user` | Requests delegation to ralphex, reviews handoff results |
| `cpt-studio-actor-ai-agent` | Discovers ralphex, compiles export plan, invokes delegation, reports handoff |
| `cpt-studio-actor-studio-cli` | Routes delegation commands for the `cf-ralphex` skill |

### 1.4 References

- **PRD**: [PRD.md](../PRD.md) — `cpt-studio-fr-core-workflows`, `cpt-studio-fr-core-execution-plans`, `cpt-studio-fr-core-agents`
- **Design**: [DESIGN.md](../DESIGN.md) — `cpt-studio-component-skill-engine`, `cpt-studio-component-config-manager`, `cpt-studio-seq-ralphex-delegation`
- **ADR**: [ADR-0018](../ADR/0018-cpt-studio-adr-ralphex-delegation-skill-v1.md) — `cpt-studio-adr-ralphex-delegation-skill`
- **Dependencies**: `cpt-studio-feature-execution-plans`, `cpt-studio-feature-version-config`
...
- **DATA**: No persistent business-data model or migration strategy; config persistence is delegated to `cpt-studio-feature-version-config`
...
- Vendoring ralphex into the Studio Python package or making it a required runtime dependency
- Copying the SDLC kit (rules.md, checklist.md, kit workflows) into `.ralphex/` as a source of truth
- Replacing Studio generate/analyze/plan workflows with ralphex equivalents
- Replacing host-tool-native subagents (`cpt-studio-feature-subagent-registration`) with ralphex
- Managing ralphex internal configuration, dashboard, or review pipeline beyond what Studio needs for delegation handoff
...
- [x] `p1` - **ID**: `cpt-studio-flow-ralphex-delegation-discover`
...
- [ ] `p1` - **ID**: `cpt-studio-flow-ralphex-delegation-export`

**Actor**: `cpt-studio-actor-user`

**Success Scenarios**:
- User requests delegation of a Studio plan → plan compiled into ralphex Markdown plan under `docs/plans/`
- User requests delegation of a single phase → phase compiled into a focused ralphex plan

**Error Scenarios**:
- No active Studio plan for the target task → error with hint to run `studio-plan` first
...
2. [ ] - `p1` - Agent loads plan manifest from `{cf-studio-path}/.plans/{task-slug}/plan.toml` - `inst-load-manifest`
3. [ ] - `p1` - Agent runs `cpt-studio-algo-ralphex-delegation-compile-plan` to produce ralphex-compatible Markdown plan - `inst-compile-plan`
...
5. [ ] - `p1` - **IF** Studio needs custom ralphex review prompts or agents, generate bounded `.ralphex/` overrides as derived artifacts - `inst-generate-overrides`
...
- [ ] `p1` - **ID**: `cpt-studio-flow-ralphex-delegation-execute`
...
**Steps**:
1. [ ] - `p1` - Agent validates ralphex availability via `cpt-studio-algo-ralphex-delegation-validate` - `inst-validate-ralphex`
...
- [x] `p1` - **ID**: `cpt-studio-flow-ralphex-delegation-handoff`
...
3. [x] - `p1` - Agent runs deterministic validation commands from the original Studio plan against the current working tree - `inst-run-validation`
...
- [x] `p1` - **ID**: `cpt-studio-algo-ralphex-delegation-discover`
...
- [x] `p1` - **ID**: `cpt-studio-algo-ralphex-delegation-validate`
...
- [ ] `p1` - **ID**: `cpt-studio-algo-ralphex-delegation-compile-plan`

**Input**: Studio plan manifest (`plan.toml`), phase files, kit dependencies
...
3. [ ] - `p1` - Generate `## Validation Commands` section from Studio's deterministic validation contract (collected from phase acceptance criteria and kit constraints) - `inst-gen-validation`
4. [ ] - `p1` - **FOR EACH** target phase, run `cpt-studio-algo-ralphex-delegation-map-phase` to produce `### Task N:` block - `inst-loop-phases`
...
- [ ] `p1` - **ID**: `cpt-studio-algo-ralphex-delegation-map-phase`

**Input**: Single Studio phase file content, phase metadata
...
- [x] `p1` - **ID**: `cpt-studio-state-ralphex-delegation-lifecycle`
...
- [x] `p1` - **ID**: `cpt-studio-dod-ralphex-delegation-discovery`
...
**Implements**:
- `cpt-studio-flow-ralphex-delegation-discover`
- `cpt-studio-algo-ralphex-delegation-discover`

### Diagnostics and Availability Validation

- [x] `p1` - **ID**: `cpt-studio-dod-ralphex-delegation-diagnostics`

The skill MUST validate ralphex availability before any delegation attempt. If ralphex is missing or incompatible, the skill MUST provide diagnostic output with platform-appropriate installation guidance (Homebrew on macOS, `go install`, binary releases) without altering baseline Studio behavior.

**Implements**:
- `cpt-studio-algo-ralphex-delegation-validate`

### Optional Bootstrap

- [x] `p1` - **ID**: `cpt-studio-dod-ralphex-delegation-bootstrap`
...
- `ralphex --init` MUST NOT run automatically as a side effect of delegation or any other Studio workflow.
...
- [ ] `p1` - **ID**: `cpt-studio-dod-ralphex-delegation-modes`
...
- **Review-only mode**: `ralphex --review [plan.md]` — review committed changes on the current feature branch against the default branch (`git diff master...HEAD`); the optional plan file provides supplemental context only; precondition: changes must be committed on the feature branch. When review mode is delegated via `run_delegation()`, a Studio-derived review override is automatically generated at `.ralphex/prompts/studio-review-override.md` before invoking ralphex. This override routes review work into Studio analyze methodology (code-review and prompt/instruction-review branches) with bounded scope, completion gates, residual-risk reporting, and remediation-prompt obligations. The override references canonical Studio sources by path and is regenerated on every review-mode delegation.
...
**Implements**:
- `cpt-studio-flow-ralphex-delegation-execute`

### Post-Run Handoff

- [x] `p1` - **ID**: `cpt-studio-dod-ralphex-delegation-handoff`

The skill MUST report delegation results after ralphex completes: exit status, output references, completed plan location (`{plans_dir}/completed/`, managed by ralphex), and validation outcome. The skill MUST run the deterministic validation commands originally supplied by Studio for the delegated task to verify execution correctness independently of ralphex's own validation.

**Implements**:
- `cpt-studio-flow-ralphex-delegation-handoff`

## 6. Implementation Modules

| Module | Path | Responsibility |
|--------|------|----------------|
| ralphex Delegation Skill | `skills/.../agents/cf-ralphex.md` (skill entry point) | Discovery, export, delegation, handoff orchestration |
| Plan Export Compiler | `skills/.../scripts/studio/ralphex_export.py` | Studio plan → ralphex Markdown plan compilation |

## 7. Acceptance Criteria

- [ ] `cf-ralphex` skill discovers `ralphex` on PATH and persists resolved path in `core.toml`
...
- [ ] Studio plan outputs are exported into ralphex-compatible Markdown plans with `## Validation Commands` and `### Task N:` sections
- [ ] Export target directory is resolved from ralphex config precedence, not hardcoded or Studio-owned
- [ ] Exported plans contain only bounded SDLC slices, not the entire kit
- [ ] One Studio phase maps to one `### Task N:` block (or small contiguous group)
...
- [ ] Post-run handoff reports status, output refs, and re-runs Studio validation commands
- [ ] Integration is fully optional: projects without `ralphex` use normal Studio workflows with zero behavioral change
- [ ] No Studio SDLC assets (rules.md, checklist.md, kit workflows) are duplicated into `.ralphex/` as a parallel source of truth
