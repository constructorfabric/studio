...
**Output**: Per workflow, the files reached, the declared stops among them, the emit sites that do not stop, and the halts reached with no menu open; across the tree, how many workflows can reach each menu, which menus fire unconditionally, **how much of the total reachability the ten most-reachable menus carry**, and everything that would make those counts a floor rather than a total — files that could not be read, `LOAD` targets missing from the tree, and menu names two files both define
...
7. [x] - `p1` - Say out loud whatever would make the counts a floor rather than a total — files that could not be read, targets missing from the tree, names two files both define — since a caller reading only the numbers would take an under-count for an improvement - `inst-surface-warn`
