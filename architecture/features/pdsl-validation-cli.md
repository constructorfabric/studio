...
2. [x] - `p1` - **FOR EACH** block in source order - `inst-foreach-block`
   ...
   4. [x] - `p1` - Accept a MENU's `TYPE` only as one literal enum token per menu and only inside the declaration region: reject an interpolation, a variable, a trailing condition, a repeat, and a declaration that is outside a menu, nested in its body or trailing it; treat an absent `TYPE` as valid; bound author text interpolated into a finding. **Reject a gate declared as the auto-proceeding type unless it is registered as reviewed for that**: it is the one type that answers for the user, and the blocked-action invariants say which gates must never do so in categories a lint cannot decide — matching those categories by their own vocabulary refuses every menu in the corpus, which would disable the type rather than guard it. So the check asks whether the gate is registered, which it can answer, rather than whether it is dangerous, which it cannot; an unregistered declaration fails closed, leaving the gate asking - `inst-gate-declaration-literal`
