...
**Steps**:
1. [x] - `p1` - Load the rule registry from `studio.utils.pdsl` using stable `PDSL100`, `PDSL200`, `PDSL300`, `PDSL400`, `PDSL500`, `PDSL600`, `PDSL700`, and `PDSL710` rule bands - `inst-load-rule-registry`
2. [x] - `p1` - **FOR EACH** block in source order - `inst-foreach-block`
   ...
   3. [x] - `p1` - Open a MENU's declaration region at its header and close it at the first section that is not `TITLE`, `TYPE`, or `SHAPE` (issue #186); learn that menu's sub-header indent from its first sub-header, measured per menu, and read any line indented deeper as continuation text of the header above it rather than as a header of its own -- leaving menu scope itself untouched so pre-existing menu checks are unaffected. `TYPE` and `SHAPE` share this one region and neither ends the other's -- `inst-gate-declaration-region`
   4. [x] - `p1` - Accept a MENU's `TYPE` (`PDSL700`/`701`/`702`) or `SHAPE` (`PDSL710`/`711`/`712`, issue #186) only as one literal enum token per menu and only inside the declaration region: reject an interpolation, a variable, a trailing condition, a repeat, and a declaration that is outside a menu, nested in its body or trailing it; treat an absent declaration as valid; bound author text interpolated into a finding - `inst-gate-declaration-literal`
   5. [x] - `p1` - Within the declaration region, report a sub-header that is a near-miss of the gate keyword (`PDSL703`) or the shape keyword (`PDSL713`, issue #186) by edit distance including transposition, discarding decoration and normalizing zero-width and confusable characters rather than enumerating the forms -- a misspelling, a miscasing, a decorated form, a separator PDSL does not use or none at all, or a rejected alternative -- while reading as prose every other unrecognized sub-header, any line with neither a separator nor a valid value, and any lower- or mixed-case candidate whose value is not one of that header's valid tokens - `inst-gate-header-near-miss`
...
- [ ] Findings use stable rule bands `PDSL100` for fences, `PDSL200` for starters or keywords, `PDSL300` for duplicate names, `PDSL400` for menu numbering, `PDSL500` for deterministic local semantics, `PDSL600` for compactness caps, `PDSL700` for declared gate risk, and `PDSL710` for declared menu shape (issue #186)
