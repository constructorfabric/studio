...
- The `AGENTS.md`/`CLAUDE.md` managed block is a single project-wide resource shared by all harnesses, so it is retained while **any** in-scope harness is still in `FileFallback` (for example a harness whose hook entry has been written but not yet observed to run). It is removed only when no in-scope harness depends on it. This is another harness's live state, not residual state for the disabled harness.
...
|---------|-----------------|-----------------------|---------------|----------------------|
| `claude` | An entry appended under the `SessionStart` event key of the `hooks` object in `.claude/settings.json` | The `SessionStart` event key, with the harness's startup matcher | A command-type entry whose command invokes Studio's reserved hook script | The reserved Studio-owned hook-script path the command invokes — this format has no per-entry name field, so the reserved path is the identifier |
...
**Output**: final per-harness outcome records (routing_mode, hook_path or the pair of file_marker_paths, warnings and errors — field schema in `DESIGN.md`'s `HarnessRoutingOutcome`), with state persisted for every harness where the persistence write itself succeeds, and a `level: error` `warnings` entry reported where it does not — except a harness being promoted from `file` to `hook` this run, whose `routing_mode` is itself revised back to `file` when that specific persist fails (see the promotion-before-removal ordering below)
...
**Promotion-before-removal ordering**: a harness being promoted from FileFallback to HookInstalled this run (pass 1 returned `target_mode = hook` for a harness whose persisted state going into this run was `file`) is the one case where the shared-marker decision and state-file persistence are coupled. If the marker were removed before that harness's `hook` state is durably on disk, the harness would sit in a window where neither channel delivers: the marker is gone, and the hook stays receipt-only (silent) until the persisted mode actually reads `hook` (`cpt-studio-dod-hook-based-session-routing-payload-transport`). Step 1 below therefore durably persists a promotion **before** the dependency set (step 2) is built, and revises the target mode back to `file` — keeping the harness a marker dependent — whenever that specific persist fails, so an undurable promotion can never cause the marker to be removed out from under it.

**Steps**:
1. [ ] - `p1` - **FOR EACH** in-scope harness whose pass-1 target mode is `hook` **AND** whose persisted state going into this run was `file` (i.e. this run is promoting it from FileFallback to HookInstalled) - `inst-promote-persist-first`
   1. [ ] - `p1` - **TRY** persist `routing_mode: hook` to that harness's own state file now, before any shared-marker decision is made - `inst-promote-persist-attempt`
      ...
      2. [ ] - `p1` - **ELSE** the harness's `routing_mode: hook` is now durably persisted; step 5 below re-affirms the same value for it (idempotent) rather than persisting it again for the first time - `inst-promote-persist-succeeded`
...
4. [ ] - `p1` - **IF** the dependency set is empty - `inst-if-no-dependents`
...
5. [ ] - `p1` - **ELSE** - `inst-else-has-dependents`
...
6. [ ] - `p1` - **FOR EACH** harness affected by this run — every harness the run touched, **plus** every in-scope harness the run did not select whose state this run's shared-marker outcome changed (e.g. an unselected dependent marked `errored` by a failed shared-marker write on an `--agent`-scoped run) - `inst-for-each-finalize`
...
7. [ ] - `p1` - **RETURN** the finalized outcome records - `inst-return-final-outcomes`
...
- **Shared marker retention**: because the file marker is project-wide rather than harness-owned (`cpt-studio-dod-hook-based-session-routing-fallback`), "routing off" for one harness **MUST NOT** remove the marker while another in-scope harness still depends on it. With every in-scope harness now hook-capable by default, this case narrows to harnesses currently in `FileFallback` — a hook awaiting its first observed execution, a harness whose hook stopped verifying, or an installed client whose hook API is unavailable — but it remains reachable and is not claimed away.
...
────────────────────────────────────────
   ...
   (untouched harnesses: last known mode, or "depends" if undeterminable)
                          ...
                          v
        ┌─────────────────────────────────────────┐
        │ For each harness promoted file→hook this │
        │ run: persist routing_mode=hook NOW, before│
