...
significant_changes:
  - version: 0.8.0
    date: 2026-09-25
    summary: Round-10 PR review fixes — extend the read-state algorithm's and the state-file DoD's "treated as absent" rule to a parseable, current-schema-version file whose `routing_mode` is missing or outside the four serialized values, so inspection and the hook's own execution-time reader genuinely agree; add an explicit crash-safety paragraph covering process interruption (not write failure) between any two of the hook-entry, receipt, marker, and state-file writes, grounded in per-write atomicity and read-state's derive-from-resources behavior; and back the "gitignored via the existing managed .gitignore block" claim with a concrete new step in core-infra's gitignore-footprint algorithm, adding `cpt-studio-feature-core-infra` to this feature's dependencies.
  - version: 0.7.0
    date: 2026-09-24
...
- **Dependencies**: `cpt-studio-feature-agent-integration` (per-(tool,provider) matrix and generation pipeline this feature reuses), `cpt-studio-feature-subagent-registration` (adjacent agent-integration surface, not duplicated here), `cpt-studio-feature-core-infra` (this feature's per-harness state file and execution receipt are kept out of version control via core-infra's `cpt-studio-algo-core-infra-gitignore-footprint` mechanism, not a gitignore mechanism of this feature's own — `cpt-studio-dod-hook-based-session-routing-state-file`)
...
- The persisted state file for a harness is unparseable, carries an unrecognised schema version, or (even though parseable and current) carries a missing or invalid `routing_mode`; it is treated as absent and the state is re-derived from installed resources.
...
6. [ ] - `p1` - **IF** the dependency set is empty - `inst-if-no-dependents`
   ...
   2. [ ] - `p1` - **IF** removing the block from either file fails (permission denied, read-only tree, or any other filesystem error) - `inst-if-remove-failed`
      1. [ ] - `p1` - Roll the pair back to its pre-run content where possible, and set routing_mode = `errored` — not `off` — for every harness this run resolved to `off`, attaching the underlying OS error text, so a removal that did not actually happen is never finalized as a clean disablement; if this rollback attempt itself also fails, see "When the rollback attempt itself also fails" below - `inst-mark-remove-error`
7. [ ] - `p1` - **ELSE** - `inst-else-has-dependents`
   ...
   2. [ ] - `p1` - **IF** writing either file fails (permission denied, read-only tree, no space, or any other filesystem error) - `inst-if-file-write-failed`
      1. [ ] - `p1` - Roll the pair back to its pre-run content where possible, and set routing_mode = `errored` for every harness in the dependency set, attaching the underlying OS error text so the maintainer sees the real cause rather than a bare `off` or `file`; if this rollback attempt itself also fails, see "When the rollback attempt itself also fails" below - `inst-mark-write-error`
8. [ ] - `p1` - **FOR EACH** harness affected by this run — every harness the run touched, **plus** every in-scope harness the run did not select whose state this run's shared-marker outcome changed (e.g. an unselected dependent marked `errored` by a failed shared-marker write on an `--agent`-scoped run); Windsurf is never in this set, since it has no state file and no outcome record - `inst-for-each-finalize`
   ...
   3. [ ] - `p1` - **IF** routing_mode = `file` - `inst-if-final-file`
      1. [ ] - `p1` - Append a `level: warning` entry to the outcome record's `warnings` array, reusing the omit-when-empty presence convention from the CLI's existing `warnings` array shape (as in `workspace-info`'s `config_warnings`; the `{level, reason}` element shape is a deliberate divergence from that flat `string[]` precedent, per `HarnessRoutingOutcome`'s field schema in DESIGN.md), with a one-clause `reason` drawn from the fixed limitation-reason enum (see Section 7, item c) - `inst-append-fallback-warning`
...
**Rule — `routing_mode` picks the primary line, `warnings` is rendered in full, independently**: `routing_mode` decides only the row's primary status line (and its color/marker); it is never used to select *which* `warnings` entries get shown. Every entry in a `warnings` array, of any `level` and regardless of how many entries there are, gets its own visible line with its own severity marker in the human table — entries are never collapsed into one line, and no entry is ever dropped because another entry, or the row's `routing_mode`, already "covered" that harness. This matters concretely for a `file` outcome that is a failed `hook` promotion (DESIGN.md's `HarnessRoutingOutcome` field schema): its `warnings` array carries **two** entries — a `level: warning` fixed-enum fallback reason (e.g. `pending-first-run`) **and** a `level: error` entry with the underlying OS error text for the persist failure — and both **MUST** render as separate visible lines, not just the one implied by `routing_mode = file`.
...
1. [ ] - `p1` - **FOR EACH** outcome record - `inst-summary-for-each`
   ...
   2. [ ] - `p1` - **IF** routing_mode = `file` (fallback in effect) - `inst-render-if-fallback`
      1. [ ] - `p1` - Render the row's primary line as visually distinct and warning-level - `inst-render-fallback-warning-line`
   3. [ ] - `p1` - **IF** routing_mode = `errored` - `inst-render-if-errored`
      1. [ ] - `p1` - Render the row's primary line as error-level, and contribute to the command's existing `PARTIAL` result contract rather than being reported as a successful install - `inst-render-error-line`
   4. [ ] - `p1` - **IF** the outcome carries a `warnings` array (present per the field-schema presence rule; never inferred solely from `routing_mode`) - `inst-render-if-warnings`
      1. [ ] - `p1` - **FOR EACH** entry in `warnings`, in array order - `inst-render-warnings-for-each`
         1. [ ] - `p1` - Render that entry as its own visible line, carrying its own `level`-specific marker (`warning` or `error`) and its `reason` text, distinct from and in addition to the row's primary line above — never merged into the primary line, never omitted, and never limited to only the first entry or only the entry matching `routing_mode` - `inst-render-warning-entry`
...
2. [ ] - `p1` - Read the harness's own state file, treating it as absent when it is missing, unparseable, carries a `schema_version` this Studio version does not recognise, **or** — for an otherwise parseable, current-schema-version file — has a `routing_mode` that is missing or is not one of the four serialized values `off`/`file`/`hook`/`errored` (for example a hand-edited or legacy value such as `hooked`) - `inst-read-state-file`
...
**Crash-safety across the multi-resource write sequence**: a single `generate-agents` run touches several independent resources per harness in sequence — the hook config entry, the execution receipt, the shared `AGENTS.md`/`CLAUDE.md` marker pair, and finally the per-harness state file. The failure modes documented elsewhere in this section and in `cpt-studio-dod-hook-based-session-routing-write-failure` all cover one of those writes *failing* (permission denied, disk full, and the like); this paragraph covers the **process itself** being killed or crashing **between** two of those writes, with no individual write in progress actually failing. That interruption is safe at any point in the sequence, and relies on nothing beyond two facts already established elsewhere in this document, never a separate mechanism: (a) each single-file write this spec documents is atomic at the file level — the state file (`cpt-studio-dod-hook-based-session-routing-state-file`) is always written to a same-directory temporary file and atomically renamed over the target, never truncated in place, so a reader (including a concurrently firing hook) never observes a torn or partially written state file, only ever the complete pre-crash content or the complete post-crash content; the shared `AGENTS.md`/`CLAUDE.md` pair is written as two separate file operations, so a crash between them can leave the pair momentarily diverged, but that divergence is not a new failure mode — it is the same "partial state left by a manual edit or an interrupted earlier run" case the migration note above already treats as prior FileFallback and reconciles by restoring both files to the computed target on the next run; and (b) `cpt-studio-algo-hook-based-session-routing-read-state` never trusts the state file as the sole source of truth — every read re-derives the harness's actual state from the installed resources (hook entry, receipt, marker) and treats the persisted state file only as a cross-check. Consequently a crash mid-sequence leaves each already-completed write intact, leaves each not-yet-attempted write simply not done, and leaves at most the marker pair briefly diverged in a way this document already knows how to reconcile; at worst the state file is one run behind the resources actually on disk, which the next `generate-agents` run's derive-and-persist steps or the next `cfs agents` read self-corrects by re-deriving from resources rather than by trusting stale bookkeeping. This is the complete recovery mechanism this design relies on for crash safety across the sequence — there is no separate journal, transaction log, or multi-file atomic-commit protocol spanning the hook entry, receipt, marker pair, and state file together, and none is needed given (a) and (b).
...
**Execution receipt is gitignored, per-machine**: the per-harness execution receipt (`cpt-studio-dod-hook-based-session-routing-payload-transport`) **MUST** be treated the same way as the state file for git-tracking purposes: it **MUST** be covered by the same managed `.gitignore` block, never committed — concretely, the same `cpt-studio-algo-core-infra-gitignore-footprint` step `inst-ignore-routing-state` named above, added in anticipation of this feature's exact receipt filename once Section 7 item (a) settles it. Both are local proof-of-installation artifacts describing *this machine's* delivery state, not project configuration, and that is why neither is tracked. This decision is deliberate, not an oversight, and resolves the git-tracking half of Section 7 item (a): if the receipt were committed, one developer's "the hook fired on my machine" proof would be silently inherited by every other clone, satisfying the promotion gate for teammates whose own client may not even have the hook working — which would defeat the entire purpose of a receipt, since it is supposed to prove execution actually happened on the machine that reports `HookInstalled`, not merely that some machine, at some point, saw it fire.

**Consequence accepted — a fresh clone re-enters the pending-first-run window**: because the receipt is gitignored while the hook-config entry that names it is written into a **tracked** file (the harness's hook config, e.g. `.claude/settings.json`), a fresh clone of an already-`HookInstalled` project starts with a verified Studio-owned hook entry present (inherited from the clone) but no local receipt (never inherited). `cpt-studio-algo-hook-based-session-routing-read-state` therefore derives `FileFallback` for that developer until their own client fires the hook once and stamps a fresh local receipt — this is not a new special case; it is the same receipt-only/pending-first-run window `cpt-studio-dod-hook-based-session-routing-payload-transport` already documents, just entered by cloning instead of by a fresh install. The shared `AGENTS.md`/`CLAUDE.md` marker reinstatement this causes — present again in the fresh clone's working tree until that developer's promotion completes — is bounded, expected churn in the same category as the already-documented migration-from-pre-existing-file-injection behavior above, **not** unbounded churn: it is scoped to that one developer's own clone and their own eventual local promotion; it never touches, downgrades, or re-derives any *other* developer's already-verified `HookInstalled` state, since each clone's derivation depends only on the resources and receipt present in that clone.

The state file **MUST** carry a `schema_version` field. A state file that is unparseable, or whose `schema_version` this Studio version does not recognise, **MUST** be treated exactly as an absent file: the state is re-derived from installed resources and the file is rewritten from that derivation on the next `generate-agents` run, rather than failing the run or being partially trusted. The **same** treatment **MUST** apply to a state file that parses cleanly and carries a recognised `schema_version` but whose `routing_mode` field is missing or is not one of the four serialized values `off`/`file`/`hook`/`errored` — a hand-edited file, a legacy value from before this feature shipped, or a typo such as `hooked` is **not** partially trusted for the fields that do parse; the whole file is treated as absent and the state is re-derived and rewritten exactly as for the unparseable/unrecognised-`schema_version` case. The **same** absent-treatment rule **MUST** apply to the hook command's own execution-time read of this file, where "absent" resolves to the receipt-only, emit-nothing phase (`cpt-studio-dod-hook-based-session-routing-payload-transport`); the two readers never disagree about what an unrecognised file means — this is the same absent-file rule stated once here and applied identically by `cpt-studio-algo-hook-based-session-routing-read-state` step `inst-read-state-file` and by the execution-time reader.
...
The system **MUST** flag any harness that falls back to file injection with a warning-level, visually distinct line in the summary, reusing the omit-when-empty presence convention already used in the CLI's `warnings` array shape (as in `workspace-info`'s `config_warnings`; the `{level, reason}` element shape is a deliberate divergence from that flat `string[]` precedent, not a reuse of `cfs info --json`, which has no `warnings`-like field at all — see `HarnessRoutingOutcome`'s field schema in DESIGN.md). This warning-level line is the row's **primary** status line, driven by `routing_mode`; it is separate from, and does not substitute for, rendering every other entry in that harness's `warnings` array (per `cpt-studio-algo-hook-based-session-routing-render-summary`'s render-all-entries rule) — a `file` outcome that is also a failed `hook`-promotion persist carries a second, `level: error` entry that **MUST** render as its own line too, not be collapsed into this one.
...
- [ ] A `file` outcome that is a failed `hook` promotion (`routing_mode: file` whose `warnings` array carries both a `level: warning` fixed-enum fallback reason and a `level: error` entry with the underlying OS error text for the persist failure) renders **both** entries as separate, distinctly-marked lines in the default human table, and both are also present in `--json` — the fallback reason is never the only line shown, and the persist-failure error is never dropped or merged into it.
- [ ] The execution receipt is covered by the managed `.gitignore` block and is never present in `git status`/`git diff` as a tracked or stageable file, matching the per-harness state file's treatment.
- [ ] A fresh clone of a project where a teammate's machine already reached HookInstalled for some harness — tracked hook-config entry present, but no local execution receipt, since the receipt was never committed — is derived as FileFallback for that harness on the cloning developer's own machine, and is promoted to HookInstalled only after that developer's own client fires the hook and stamps a fresh local receipt; this is reported as the ordinary pending-first-run reason, not as an error or an inconsistency, and does not alter or re-derive any other clone's already-verified HookInstalled state.
...
- [ ] **(a) Execution-receipt lifetime**: The fallback gate depends on a per-harness execution receipt proving the current hook entry has run (`cpt-studio-dod-hook-based-session-routing-payload-transport`). Its exact location, format, and staleness policy need a decision — in particular whether a receipt older than some interval should demote a harness from HookInstalled back to FileFallback, and how the receipt binds to a specific entry revision so editing the entry invalidates it. **Decided, not open**: the receipt's **git-tracking status** — see `cpt-studio-dod-hook-based-session-routing-state-file`'s "Execution receipt is gitignored, per-machine" paragraph. The receipt **MUST** be gitignored, matching the per-harness state file's existing treatment, precisely **because** it is proof that the hook fired *on this machine*; committing it would let one developer's local proof satisfy the promotion gate for every other clone, which is false — a teammate's client may not even have the hook working. What remains open here is only location-within-the-gitignored-config-dir, exact format, and staleness policy, not whether it is tracked.
