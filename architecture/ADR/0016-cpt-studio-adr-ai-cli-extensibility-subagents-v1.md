...
**ID**: `cpt-studio-adr-ai-cli-extensibility-subagents`
...
- [Canonical Format: Claude Code](#canonical-format-claude-code)
  ...
  - [Example: `cf-pr-review` Across All Tools](#example-cf-pr-review-across-all-tools)
...
Today, `cfs generate-agents` generates a single monolithic skill integration per tool. All Studio work — generation, validation, review — runs inside a single agent context with full tool access. This creates two problems:
...
1. **Generate subagent definitions** — extend `cfs generate-agents` to produce isolated subagent definitions alongside existing skills and workflows
2. **Status quo** — continue with the single-agent skill integration; do not generate subagents

## Decision Outcome

Chosen option: **Option 1 — Generate subagent definitions**. Subagents are standard extensibility primitives exposed by the host tools Studio already integrates with. Not adopting them means leaving value on the table that the host tools are explicitly designed to provide.

The `cfs generate-agents` command generates two purpose-built subagent definitions for each supported tool:

| Subagent | Mode | Isolation | Purpose |
|----------|------|-----------|---------|
| `cf-codegen` | read-write | worktree (Claude Code) | Fully-specified code generation tasks: the parent agent formulates a complete specification and delegates implementation to a subagent that works in an isolated copy of the repo |
| `cf-pr-review` | read-only | none | Structured checklist-based PR review: the subagent reads the diff and produces a structured review without write access |
...
Subagent definitions are config-driven: built-in defaults via `_default_subagents()` can be overridden through `studio-agents.json` or kit-level `agents.toml` files. This enables teams to define custom subagents for their domain (e.g., a `studio-migration` agent for database schema changes).
...
* Good, because least privilege is enforced per-subagent — `cf-pr-review` cannot write files; `cf-codegen` works in an isolated worktree
...
* Neutral, because Windsurf users get no subagent support (tool limitation, not a Studio limitation)
...
- `cfs agents --agent claude` generates `cf-codegen.md` and `cf-pr-review.md` in `.claude/agents/` with correct YAML frontmatter (isolation, disallowed tools)
- `cfs agents --agent cursor` generates equivalent subagent files in `.cursor/agents/` with `readonly: true` for pr-review
- `cfs agents --agent copilot` generates `.agent.md` files in `.github/agents/`
- `cfs agents --agent openai` generates a single TOML with two agent sections in `.codex/agents/`
- `cfs agents --agent windsurf` skips subagent generation with a reason in JSON output
- Re-running subagent generation is idempotent (no spurious updates)

## Canonical Format: Claude Code

Subagents are **defined once** in `agents.toml` using semantic properties and **generated for all supported tools** automatically. Claude Code has the richest subagent definition format and supports every semantic property Studio needs, so we treat it as the **canonical format** — the full-fidelity representation. Every other tool's output is an adaptation: same semantic content, projected into that tool's native format with graceful degradation for unsupported capabilities.
...
---
name: cf-codegen
description: Studio code generator. Use when requirements are fully specified...
...
|----------|---------|---------|
| `name` | Agent identifier | `cf-codegen` |
...
### Example: `cf-pr-review` Across All Tools

The same semantic definition (`mode: readonly`, `model: fast`, `isolation: false`) produces four different outputs:

**Claude Code** (`.claude/agents/cf-pr-review.md`):

```yaml
---
name: cf-pr-review
description: Studio PR reviewer...
...
**Cursor** (`.cursor/agents/cf-pr-review.md`):
...
**GitHub Copilot** (`.github/agents/cf-pr-review.agent.md`):
...
**OpenAI Codex** (`.codex/agents/studio-agents.toml`, shared with all agents):

```toml
[agents.studio_pr_review]
description = "Studio PR reviewer..."
...
Extend `cfs generate-agents` to produce isolated subagent definitions alongside existing skills and workflows.
...
AI CLI tools expose **hooks** — deterministic shell commands that fire on agent lifecycle events. These are powerful for enforcing validation, injecting context, and controlling agent behavior. However, this ADR scopes hook usage to **subagent-level hooks only** — hooks that fire when Studio subagents start or stop — and defers project-level hook generation (e.g., `PreToolUse` hooks on every file edit) to a future decision.

**Why subagent-level hooks matter**: Claude Code supports `SubagentStart` and `SubagentStop` hook events that fire when a subagent is spawned or finishes. These can inject Studio-specific context into a subagent at launch (e.g., which kit is active, which artifact paths to validate) without polluting the parent agent's context. GitHub Copilot has equivalent `subagentStart`/`subagentStop` events.

**What is in scope for this ADR:**

- Subagent definitions reference the correct SKILL.md and workflow entry points in their prompts — this is the primary mechanism for Studio-aware subagent behavior
- Tools that support subagent lifecycle hooks (Claude Code, Copilot) can use `SubagentStart` to inject additional context into Studio subagents at spawn time
- Subagent prompt files can declare hooks in their frontmatter (Claude Code supports this) for subagent-scoped behavior

**What is deferred:**

- Project-level `PreToolUse`/`PostToolUse` hooks that run `cfs validate` on every file edit — this requires careful design around config merging, fail-open semantics, and multi-tool format differences
- Project-level `SessionStart` hooks for injecting Studio context into all sessions
- A `cfs hooks install` / `uninstall` CLI command
...
Subagents are the host tool's native mechanism for **context management** and **conversation orchestration**. Without them, every Studio task — code generation, validation, review — runs inside a single agent context. This creates compounding problems:
...
* **Overprivilege** — a monolithic agent has full tool access for every task. Subagents scope tool permissions per role: `cf-pr-review` is read-only (cannot write files); `cf-codegen` uses worktree isolation (cannot modify the working tree directly). This enforces least privilege at the tool level, reducing the blast radius of agent errors.

These are not hypothetical benefits — they are the reason the host tools built subagent support in the first place. Studio should use the primitives its host tools provide.

### Why Not Project-Level Hooks Yet

Project-level hooks (e.g., running `cfs validate` on every `Write`/`Edit` via `PreToolUse`) are valuable but introduce complexity that is best addressed separately:
...
* **Fail-open design** — hook scripts must exit 0 (allow) when `cpt` is not found, when the file is not a Studio artifact, or when validation encounters a runtime error. Getting fail-open semantics right across all tools requires careful testing.
...
The single-agent skill integration works, but it leaves the host tool's extensibility surface unused. Every tool Studio integrates with (Claude Code, Cursor, Copilot, Codex) already exposes subagent definitions. Not adopting them means:

* Accepting context saturation as a permanent limitation
* Accepting overprivilege as a permanent limitation
* Doing more work inside Studio's skill layer to approximate what the host tool already provides natively

## Maintaining the Support Matrix

The AI CLI tool landscape is evolving rapidly. Tool vendors add subagent capabilities, change frontmatter schemas, and introduce new features on their own release cadences. To keep Studio's subagent generation correct and current:
...
To test against a real tool, generate the subagent files (`cfs generate-agents --agent <tool>`) and verify the host tool recognizes the generated definitions (e.g., Claude Code should list the subagents in its agent picker).

## More Information

- Related: `cpt-studio-adr-prefer-cpt-cli-for-agents` (ADR-0015) — subagent prompts reference `cpt` via the same CLI preference mechanism
- Related: `cpt-studio-adr-proxy-cli-pattern` (ADR-0007) — subagents use the `cpt` proxy for command forwarding
- Related: `cpt-studio-adr-skill-md-entry-point` (ADR-0010) — subagent prompt files reference SKILL.md as their entry point
...
* `cpt-studio-fr-core-agents` — multi-agent integration extended with subagent generation
* `cpt-studio-principle-determinism-first` — subagents enforce deterministic privilege scoping (read-only vs read-write)
* `cpt-studio-principle-plugin-extensibility` — leveraging host-tool subagent definitions as native extensibility primitives
