...
- `cfs init` and `cfs update` work without blueprint processing
...
* `cpt-studio-fr-core-kits` — Simplifies the Extensible Kit System from blueprint-based generation to direct file management
* `cpt-studio-fr-core-resource-diff` — Redefines the Resource Diff Engine as the primary kit update mechanism (file-level diff with interactive prompts)
* cpt-studio-fr-core-blueprint (see DESIGN — DEPRECATED) — Deprecates and removes the Artifact Blueprint requirement entirely
* cpt-studio-component-blueprint-processor — Removes this component from the architecture
* `cpt-studio-component-kit-manager` — Simplifies Kit Manager to file copy + file-level diff (no generation, no hash detection)
* `cpt-studio-principle-plugin-extensibility` — Redefines kit extensibility as file packages rather than blueprint packages
