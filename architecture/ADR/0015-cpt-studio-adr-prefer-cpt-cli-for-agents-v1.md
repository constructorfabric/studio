...
**ID**: `cpt-studio-adr-prefer-cpt-cli-for-agents`
...
> **Rebrand note**: Per the v1.0.0 rebrand, the canonical CLI is `cfs`; `cpt` is now a recognized alias retained for compatibility. The reasoning below predates the rebrand.

Studio's SKILL.md instructs AI agents (Claude, Cursor, Copilot, etc.) to invoke the skill engine via a raw Python path: `python3 {cf-studio-path}/.core/skills/studio/scripts/studio.py --json <command>`. This works but creates several problems: the invocation is fragile (depends on the correct `python3` being on PATH with `tomllib` support — Python 3.11+), the command is long and error-prone for LLMs to reproduce, and it looks nothing like a standard Unix tool. Meanwhile, a global `cpt` CLI proxy already exists (ADR-0003, ADR-0007) that handles interpreter resolution, version negotiation, and command forwarding — but SKILL.md never references it.

## Decision Drivers

* **LLM ergonomics** — AI agents handle short, conventional CLI commands (`cfs validate`) far more reliably than long interpreter-qualified paths; fewer tokens, fewer transcription errors
...
* Good, because agent invocations become shorter and more reliable (`cfs --json validate` vs. `python3 /long/path/studio.py --json validate`)
...
* Neutral, because users without `cpt` see a one-time installation prompt with persistent dismissal — once dismissed, the prompt does not reappear (state stored in `~/.cf-studio/cache/cpt-prompt-dismissed`)
...
- An agent session with `cpt` installed uses `cfs --json <command>` for all invocations
- An agent session without `cpt` falls back to `python3 {cf-studio-path}/.core/skills/studio/scripts/studio.py` and functions identically
...
Keep the current SKILL.md approach. All agent invocations use `python3 {cf-studio-path}/.core/skills/studio/scripts/studio.py`.
...
* Bad, because increases onboarding friction — users must install before any Studio work

## More Information

- Related: `cpt-studio-adr-pipx-distribution` (ADR-0003) — defines `pipx` as the installation mechanism for `cpt`
- Related: `cpt-studio-adr-proxy-cli-pattern` (ADR-0007) — defines the stateless proxy pattern that `cpt` implements
- The `cpt` proxy forwards all commands to the skill engine unchanged; there is zero behavioral difference between `cfs --json validate` and `python3 .../studio.py --json validate`
...
* `cpt-studio-fr-core-installer` — extends the installer story to include agent-prompt integration, not just human installation
* `cpt-studio-fr-core-skill-engine` — the skill engine is invoked the same way regardless of entry point; this decision standardizes the agent-facing entry point
* `cpt-studio-principle-determinism-first` — deterministic CLI resolution at session start; no ambiguity about which interpreter or path is used
