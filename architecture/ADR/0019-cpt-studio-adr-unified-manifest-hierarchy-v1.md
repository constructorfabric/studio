...
**ID**: `cpt-studio-adr-unified-manifest-hierarchy`
...
Studio generates agent entry points (skills, workflows, rules) for 5 AI coding tools from a hardcoded registry in `_default_agents_config()`. The subagent registration feature (commit `ffb5446`) added kit-level agent declarations via a separate `agents.toml` file. However, there is no mechanism for individual projects to:

1. Register their own skills, agents, workflows, or rules through `cfs generate-agents` (hook support is deferred to a follow-up feature)
...
* **Python stdlib only** — per `cpt-studio-constraint-python-stdlib`
...
- `cfs generate-agents --agent claude` in a multi-layer repo produces correct merged output from all discovered layers (Core, Kit, Master Repo, Repo)
- `cfs generate-agents --show-layers` shows accurate provenance for all components, including which layer contributed each entry
...
- Related: `cpt-studio-adr-extract-sdlc-kit` — kits are external GitHub packages with manifest-driven resource binding; this ADR extends that manifest format with component sections
- Related: `cpt-studio-adr-remove-system-from-core-toml` — system identity lives in `artifacts.toml`, not `core.toml`; manifests do not duplicate system identity
- Related: Subagent registration (commit `ffb5446`) — current `agents.toml` declarations migrate into manifest.toml `[[agents]]`
- Feature: `cpt-studio-feature-project-extensibility` — the feature spec implementing this decision
...
* `cpt-studio-fr-core-agents` — extends agent generation with multi-layer inputs
* `cpt-studio-fr-core-kits` — extends kit system with component declarations in manifest
* `cpt-studio-fr-core-kit-manifest` — extends manifest.toml schema to v2.0 with component sections
* `cpt-studio-principle-plugin-extensibility` — enables project-level extension of the generation pipeline
* `cpt-studio-principle-determinism-first` — pipeline is deterministic, no LLM calls
* `cpt-studio-constraint-python-stdlib` — implementation uses only Python stdlib
