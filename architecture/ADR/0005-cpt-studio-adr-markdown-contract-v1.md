...
**ID**: `cpt-studio-adr-markdown-contract`

## Context and Problem Statement

Studio needs a format for artifacts (PRD, DESIGN, ADR, FEATURE, etc.), workflows, templates, checklists, and rules. The format must be readable by humans, renderable by development platforms (GitHub, IDEs), and parseable by both deterministic scripts and AI agents.
...
* Bad, because complex nested structures (e.g., constraint definitions) are harder to express than in YAML/TOML — mitigated by using TOML for structured data (see `cpt-studio-adr-toml-json-formats`)
...
- **PRD**: `cpt-studio-fr-core-traceability` (template structure compliance)
- **DESIGN**: `cpt-studio-constraint-markdown-contract`
