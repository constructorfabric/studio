...
**ID**: `cpt-studio-adr-gh-cli-integration`

## Context and Problem Statement

Studio's SDLC kit needs to fetch PR diffs, metadata, comments, and CI status from GitHub for PR review and status workflows. The integration must work without adding HTTP client dependencies to the stdlib-only core.

## Decision Drivers

* **No pip dependencies** — core is stdlib-only (see `cpt-studio-adr-python-stdlib-only`); adding `requests` or `httpx` would violate this constraint
...
* **API abstraction** — GitHub REST/GraphQL API changes are absorbed by `gh` CLI updates, not Studio code changes
...
* Good, because `gh` is optional — Studio works without it; only PR workflows require it
* Bad, because `gh` must be pre-installed and authenticated — mitigated by `cfs doctor` checking availability
...
- **DESIGN**: `cpt-studio-component-kit-manager` (kits provide PR workflows)
- **Depends on**: `cpt-studio-adr-python-stdlib-only` (no pip dependencies constraint)
