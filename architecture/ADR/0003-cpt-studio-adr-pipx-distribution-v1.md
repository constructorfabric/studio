...
**ID**: `cpt-studio-adr-pipx-distribution`

## Context and Problem Statement

Studio needs a global installation mechanism that makes the CLI available system-wide as `cfs` (with `studio` and `cpt` as recognized aliases per ADR-0020). The installation must be a single command, isolated from project dependencies, and work across platforms.
...
* **Python ecosystem** — core is Python (see `cpt-studio-adr-python-stdlib-only`), so Python-native distribution is natural
...
Chosen: **pipx** (`pipx install git+https://github.com/constructorfabric/studio.git`).
...
* Good, because automatic PATH management for `cfs` (and alias) entry points
...
Confirmed by successful `pipx install` on Linux, macOS, and Windows with automatic `cfs` (and alias) entry point registration.
...
- **Depends on**: `cpt-studio-adr-python-stdlib-only`
