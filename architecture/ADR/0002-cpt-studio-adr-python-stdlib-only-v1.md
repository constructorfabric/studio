...
**ID**: `cpt-studio-adr-python-stdlib-only`

## Context and Problem Statement

Studio needs a runtime platform for its deterministic skill engine and CLI tool. The tool must work cross-platform (Linux, macOS, Windows), install with minimal dependencies, and avoid conflicts with project dependencies. The skill engine is copied into each project's install directory and must run without a virtual environment or pip install step.
...
* **`tomllib` availability** — Python 3.11+ includes `tomllib` in stdlib, which Studio needs for TOML config parsing (see `cpt-studio-adr-toml-json-formats`)
...
- **PRD**: `cpt-studio-nfr-simplicity` (minimal installation dependencies)
- **DESIGN**: `cpt-studio-constraint-python-stdlib`, `cpt-studio-constraint-cross-platform`
