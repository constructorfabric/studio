...
* **Stdlib availability** — Python 3.11+ includes `tomllib` for reading TOML (see `cpt-studio-adr-python-stdlib-only`)
...
cfs <command> [options]           # → human output to stderr
```
...
- **PRD**: `cpt-studio-nfr-simplicity` (intuitive config), `cpt-studio-fr-core-config`, `cpt-studio-fr-core-skill-engine` (machine-readable output)
- **DESIGN**: `cpt-studio-principle-machine-readable`, `cpt-studio-principle-tool-managed-config`
- **Depends on**: `cpt-studio-adr-python-stdlib-only` (tomllib availability)
