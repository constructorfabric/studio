...
**ID**: `cpt-studio-adr-structured-id-format`

## Context and Problem Statement

Studio's core value is linking design elements to code. This requires a unique identifier system that works across Markdown artifacts and source code files in any programming language. IDs must be human-readable, deterministically parseable, and collision-resistant across systems in a monorepo.
...
**Artifact IDs** (definitions): `**ID**: \`cpt-studio-fr-core-init\``
...
**Code tags** (references): `@cpt-studio-fr-core-init` in language-native comments
...
* Good, because human-readable — `cpt-studio-fr-core-init` tells you: system=studio, kind=functional-requirement, scope=core-init
* Good, because grep-friendly — `grep -r "cpt-studio-fr-core-init"` finds all references instantly
...
Confirmed by bidirectional traceability working across all architecture artifacts and source files using `grep -r "cpt-studio-"` for discovery.
...
- **PRD**: `cpt-studio-fr-core-traceability`
- **DESIGN**: `cpt-studio-component-traceability-engine`, `cpt-studio-principle-traceability-by-design`
